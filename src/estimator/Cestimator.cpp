...
void Cestimator::Initialize(const string& my_logfilename, const int& my_number_of_steps, const time_t& simulation_starting_time)
...
//	current_user = getlogin();
	...
	_simulation_starting_time = simulation_starting_time;
...
//			InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
		...
		string func_name = "";
#endif
		throw AngoraInvalidArgumentExceptionWithType<string>(func_name,logfilename,
			"(could not open file)");
	}

	tm *struct_simulation_starting_time = localtime(&_simulation_starting_time);
	...
	LogFile << user << " started Angora on " << str_simulation_starting_time << endl;
...
void Cestimator::MakeEstimate(const int& n)
{
	if (n==sample_simulation_size-1)
	{
		...
		write_log(_simulation_starting_time, _simulation_starting_time + estimate,estimate);	//write out timing data
	}
}
...
//			InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
	...
	actual_elapsed_time = (int)difftime(actual_finishing_time,_simulation_starting_time);  //get the running time of the simulation
