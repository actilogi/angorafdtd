...
//Definitions of the FFT wrappers (double type)
...
#include "kiss_D_fft/kiss_D_fftnd.h"
#include "kiss_D_fft/kiss_D_fftndr.h"
...
/** The internals of these functions are the same as in fft_f.cpp. **/
...
{//computes the 1D Fourier transform
	kiss_D_fft_cfg mycfg=kiss_D_fft_alloc(N_FFT,inverse_fft,NULL,NULL); //computes inverse FFT if inverse_fft is not 0  NOTE:inverse FFT is NOT scaled by 1/(N_FFT)
	//Here, we assume that kiss_D_fft_cpx is bit-compatible to the C++ complex<T> datatype
	//kiss_D_fft_cpx is just a struct with two elements of type T, so it is sufficiently simple for this assumption
	const kiss_D_fft_cpx* input_kissfft = reinterpret_cast<const kiss_D_fft_cpx*>(input.data());
	kiss_D_fft_cpx* output_kissfft = reinterpret_cast<kiss_D_fft_cpx*>(output.data());
	kiss_D_fft(mycfg,input_kissfft,output_kissfft);
	free(mycfg);
}
...
{//(Works only for even N_FFT)
 ...
 //output will be of length N_FFT
	...
	kiss_D_fftr_cfg mycfg=kiss_D_fftr_alloc(N_FFT,inverse_fft,NULL,NULL);
	const kiss_D_fft_cpx* input_kissfft = reinterpret_cast<const kiss_D_fft_cpx*>(input.data());
	kiss_D_fft_scalar* output_kissfft = reinterpret_cast<kiss_D_fft_scalar*>(output.data());
	kiss_D_fftri(mycfg,input_kissfft,output_kissfft);
	/*
...
{//(Works only for even N_FFT)
 ...
 //output will be of length N_FFT/2+1
	...
	kiss_D_fftr_cfg mycfg=kiss_D_fftr_alloc(N_FFT,inverse_fft,NULL,NULL);
	const kiss_D_fft_scalar* input_kissfft = reinterpret_cast<const kiss_D_fft_scalar*>(input.data());
	kiss_D_fft_cpx* output_kissfft = reinterpret_cast<kiss_D_fft_cpx*>(output.data());
	kiss_D_fftr(mycfg,input_kissfft,output_kissfft);
	/*
...
{//computes the 2D Fourier transform
	...
	kiss_D_fftnd_cfg mycfg=kiss_D_fftnd_alloc(dims,ndims,inverse_fft,NULL,NULL); //computes inverse FFT if inverse_fft is not 0   NOTE:inverse FFT is NOT scaled by 1/(N1*N2)
	//Here, we assume that kiss_D_fft_cpx is bit-compatible to the C++ complex<T> datatype
	//kiss_D_fft_cpx is just a struct with two elements of type T, so it is sufficiently simple for this assumption
	const kiss_D_fft_cpx* input_kissfft = reinterpret_cast<const kiss_D_fft_cpx*>(input.data());
	kiss_D_fft_cpx* output_kissfft = reinterpret_cast<kiss_D_fft_cpx*>(output.data());
	kiss_D_fftnd(mycfg,input_kissfft,output_kissfft);
...
{//(Works only for even N2)
 ...
 //output will be of dimensions N1 x N2
	const int ndims = 2;
	const int dims[ndims] = {N1,N2};
	kiss_D_fftndr_cfg mycfg=kiss_D_fftndr_alloc(dims,ndims,inverse_fft,NULL,NULL);
	const kiss_D_fft_scalar* input_kissfft = reinterpret_cast<const kiss_D_fft_scalar*>(input.data());
	kiss_D_fft_cpx* output_kissfft = reinterpret_cast<kiss_D_fft_cpx*>(output.data());
	kiss_D_fftndr(mycfg,input_kissfft,output_kissfft);
	free(mycfg); //mycfg is allocated dynamically, so free it
}
...
{//(Works only for even N2)
 ...
 //output will be of dimensions N1 x (N2/2+1)
	...
	const kiss_D_fft_cpx* input_kissfft = reinterpret_cast<const kiss_D_fft_cpx*>(input.data());
	kiss_D_fft_scalar* output_kissfft = reinterpret_cast<kiss_D_fft_scalar*>(output.data());
	kiss_D_fftndri(mycfg,input_kissfft,output_kissfft);
	free(mycfg); //mycfg is allocated dynamically, so free it
}
...
{//computes the 3D Fourier transform
	...
	kiss_D_fftnd_cfg mycfg=kiss_D_fftnd_alloc(dims,ndims,inverse_fft,NULL,NULL); //computes inverse FFT if inverse_fft is not 0   NOTE:inverse FFT is NOT scaled by 1/(N1*N2*N3)
	//Here, we assume that kiss_D_fft_cpx is bit-compatible to the C++ complex<T> datatype
	//kiss_D_fft_cpx is just a struct with two elements of type T, so it is sufficiently simple for this assumption
	const kiss_D_fft_cpx* input_kissfft = reinterpret_cast<const kiss_D_fft_cpx*>(input.data());
	kiss_D_fft_cpx* output_kissfft = reinterpret_cast<kiss_D_fft_cpx*>(output.data());
