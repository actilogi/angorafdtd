...
extern Array<float,1> eps_x,mu_y,mu_z;
extern Array<float,1> cond_e_x,cond_h_y,cond_h_z;
extern Array<float,1> mu_x,eps_y,eps_z;
extern Array<float,1> cond_h_x,cond_e_y,cond_e_z;
...
//		InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
	...
	else if (component=="Ex")
	{
		...
		RecordedPermittivityIndex = &eps_x_indices;
		RecordedPermittivityComponent = &eps_x;
		RecordedElectricConductivityIndex = &cond_e_x_indices;
		RecordedElectricConductivityComponent = &cond_e_x;
		if (section=="yz")
		{
			X1_extra = 1;
			X2_extra = 1;
	...
	else if (component=="Ey")
	{
		...
		RecordedPermittivityIndex = &eps_y_indices;
		RecordedPermittivityComponent = &eps_y;
		RecordedElectricConductivityIndex = &cond_e_y_indices;
		RecordedElectricConductivityComponent = &cond_e_y;
		if (section=="yz")
		{
			X1_extra = 0;
			X2_extra = 1;
	...
	else if (component=="Ez")
	{
		...
		RecordedPermittivityIndex = &eps_z_indices;
		RecordedPermittivityComponent = &eps_z;
		RecordedElectricConductivityIndex = &cond_e_y_indices;
		RecordedElectricConductivityComponent = &cond_e_z;
		if (section=="yz")
		{
			X1_extra = 1;
			X2_extra = 0;
	...
	else if (component=="Hx")
	{
		RecordedFieldComponent = &Hx;
		RecordedPermeabilityIndex = &mu_x_indices;
		RecordedPermeabilityComponent = &mu_x;
		RecordedMagneticConductivityIndex = &cond_h_x_indices;
		RecordedMagneticConductivityComponent = &cond_h_x;
		if (section=="yz")
		{
			X1_extra = 0;
			X2_extra = 0;
	...
	else if (component=="Hy")
	{
		RecordedFieldComponent = &Hy;
		RecordedPermeabilityIndex = &mu_y_indices;
		RecordedPermeabilityComponent = &mu_y;
		RecordedMagneticConductivityIndex = &cond_h_y_indices;
		RecordedMagneticConductivityComponent = &cond_h_y;
		if (section=="yz")
		{
			X1_extra = 1;
			X2_extra = 0;
	...
	else if (component=="Hz")
	{
		RecordedFieldComponent = &Hz;
		RecordedPermeabilityIndex = &mu_z_indices;
		RecordedPermeabilityComponent = &mu_z;
		RecordedMagneticConductivityIndex = &cond_h_z_indices;
		RecordedMagneticConductivityComponent = &cond_h_z;
		if (section=="yz")
		{
			X1_extra = 0;
			X2_extra = 1;
...
//		InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
		...
		throw AngoraInvalidArgumentExceptionWithType<string>(func_name,component,
			"(valid arguments are \"E\", \"Ex\", \"Ey\", \"Ez\", \"H\", \"Hx\", \"Hy\", or \"Hz\")");
	}
...
// cout << "I am rank " << rank << " and PassesThroughNode is " << PassesThroughNode << endl;
	...
	if (PassesThroughNode)
	{
		...
		if (movie_rank==0)
		{
			...
			//float array:
			flt4_MovieFrameArray.resize(Range(1,TotalFrameSize_X1),Range(1,TotalFrameSize_X2));	//whole frame
			flt4_MovieFrameArray = 0;
			//double array:
			...
			//float arrays:
			flt4_PartialFrameBuf_recv.resize(movie_nodes_X1,movie_nodes_X2);
			//double arrays:
			...
			for (n1=0; n1<movie_nodes_X1; n1++)
			{
				for (n2=0; n2<movie_nodes_X2; n2++)
				{
					...
					//float array:
					flt4_PartialFrameBuf_recv(n1,n2).resize
						(Range(FrameMinArray_X1(n1),FrameMaxArray_X1(n1)),Range(FrameMinArray_X2(n2),FrameMaxArray_X2(n2)));
					//double array:
		...
		else
		{
			...
			//float array:
			flt4_PartialFrameBuf_send.resize(Range(FrameMin_X1,FrameMax_X1),Range(FrameMin_X2,FrameMax_X2));	//part of the frame
			//double array:
		...
		{
			RecordPermittivity(RecordedPermittivityIndex,RecordedPermittivityComponent);
			RecordElectricConductivity(RecordedElectricConductivityIndex,RecordedElectricConductivityComponent);
		}
		else if ((component=="H")||(component=="Hx")||(component=="Hy")||(component=="Hz"))
		{
			RecordPermeability(RecordedPermeabilityIndex,RecordedPermeabilityComponent);
			RecordMagneticConductivity(RecordedMagneticConductivityIndex,RecordedMagneticConductivityComponent);
		}
#endif
		...
		//float array:
		flt4_PartialFrame.resize(Range(FrameMin_X1,FrameMax_X1),Range(FrameMin_X2,FrameMax_X2));
		//double array:
...
void Crecorder_movie::RecordPermittivity(const Array<eps_x_type,3>* PropertyIndexArray, const Array<float,1>* PermittivityArray)
{
#ifndef MPI_DISABLE
	if (PassesThroughNode)
	{
		...
		if (movie_rank==0)
		{
			...
			for (n1=0; n1<movie_nodes_X1; n1++)
			{
				for (n2=0; n2<movie_nodes_X2; n2++)
				{
					...
					//do NOT try to receive data from node 0 (yourself), place available data
					{
						PlacePermittivityFrame(PropertyIndexArray,PermittivityArray,flt4_MovieFrameArray);
					}
					else
					//receive data from other nodes
					{
						recv_coord[0] = n1; recv_coord[1] = n2;
						MPI_Cart_rank(MPI_CartMovieComm,recv_coord,&recv_rank);
						MPI_Recv(flt4_PartialFrameBuf_recv(n1,n2).data(),flt4_PartialFrameBuf_recv(n1,n2).size(),MPI_FLOAT,
							recv_rank,0,MPI_CartMovieComm,&Status);
						//place buffer into place
						flt4_MovieFrameArray(Range(FrameMinArray_X1(n1),FrameMaxArray_X1(n1)),
							Range(FrameMinArray_X2(n2),FrameMaxArray_X2(n2)))
							=flt4_PartialFrameBuf_recv(n1,n2);
					}
				}
			}
			//write out the data
			//blitz++ does row-major ordering, therefore the 2nd dimension gets written first
			MovieFile.write((char*)flt4_MovieFrameArray.data(),flt4_MovieFrameArray.size()*sizeof(flt4_MovieFrameArray(0)));
		}
		else
		{
			//send data to node 0
			PlacePermittivityFrame(PropertyIndexArray,PermittivityArray,flt4_PartialFrameBuf_send);
			MPI_Send(flt4_PartialFrameBuf_send.data(),flt4_PartialFrameBuf_send.size(),MPI_FLOAT,0,0,MPI_CartMovieComm);
		}
#endif

#ifdef MPI_USE_PARALLEL_IO
		PlacePermittivityFrame(PropertyIndexArray,PermittivityArray,flt4_PartialFrame);
		//write out the data
		//blitz++ does row-major ordering, therefore the 2nd dimension gets written first
		//use collective write to ensure that all nodes write their material properties
		MPI_File_write_all(MPIMovieFile,flt4_PartialFrame.data(),flt4_PartialFrame.size(),MPI_FLOAT,&Status);
#endif
	}
#else //MPI_DISABLE
	//no MPI, only one node
...
void Crecorder_movie::RecordElectricConductivity(const Array<cond_e_x_type,3>* PropertyIndexArray, const Array<float,1>* ElectricConductivityArray)
{
#ifndef MPI_DISABLE
	if (PassesThroughNode)
	{
		...
		if (movie_rank==0)
		{
			...
			for (n1=0; n1<movie_nodes_X1; n1++)
			{
				for (n2=0; n2<movie_nodes_X2; n2++)
				{
					...
					//do NOT try to receive data from node 0 (yourself), place available data
					{
						PlaceElectricConductivityFrame(PropertyIndexArray,ElectricConductivityArray,flt4_MovieFrameArray);
					}
					else
					//receive data from other nodes
					{
		...
		{
			//send data to node 0
			PlaceElectricConductivityFrame(PropertyIndexArray,ElectricConductivityArray,flt4_PartialFrameBuf_send);
			MPI_Send(flt4_PartialFrameBuf_send.data(),flt4_PartialFrameBuf_send.size(),MPI_FLOAT,0,0,MPI_CartMovieComm);
		}
#endif

#ifdef MPI_USE_PARALLEL_IO
		PlaceElectricConductivityFrame(PropertyIndexArray,ElectricConductivityArray,flt4_PartialFrame);
		//write out the data
		//blitz++ does row-major ordering, therefore the 2nd dimension gets written first
		//use collective write to ensure that all nodes write their material properties
		MPI_File_write_all(MPIMovieFile,flt4_PartialFrame.data(),flt4_PartialFrame.size(),MPI_FLOAT,&Status);
...
void Crecorder_movie::RecordPermeability(const Array<mu_x_type,3>* PropertyIndexArray, const Array<float,1>* PermeabilityArray)
{
#ifndef MPI_DISABLE
	if (PassesThroughNode)
	{
		...
		if (movie_rank==0)
		{
			...
			for (n1=0; n1<movie_nodes_X1; n1++)
			{
				for (n2=0; n2<movie_nodes_X2; n2++)
				{
					...
					//do NOT try to receive data from node 0 (yourself), place available data
					{
						PlacePermeabilityFrame(PropertyIndexArray,PermeabilityArray,flt4_MovieFrameArray);
					}
					else
					//receive data from other nodes
					{
		...
		{
			//send data to node 0
			PlacePermeabilityFrame(PropertyIndexArray,PermeabilityArray,flt4_PartialFrameBuf_send);
			MPI_Send(flt4_PartialFrameBuf_send.data(),flt4_PartialFrameBuf_send.size(),MPI_FLOAT,0,0,MPI_CartMovieComm);
		}
#endif

#ifdef MPI_USE_PARALLEL_IO
		PlacePermeabilityFrame(PropertyIndexArray,PermeabilityArray,flt4_PartialFrame);
		//write out the data
		//blitz++ does row-major ordering, therefore the 2nd dimension gets written first
		//use collective write to ensure that all nodes write their material properties
		MPI_File_write_all(MPIMovieFile,flt4_PartialFrame.data(),flt4_PartialFrame.size(),MPI_FLOAT,&Status);
...
void Crecorder_movie::RecordMagneticConductivity(const Array<cond_h_x_type,3>* PropertyIndexArray, const Array<float,1>* MagneticConductivityArray)
{
#ifndef MPI_DISABLE
	if (PassesThroughNode)
	{
		...
		if (movie_rank==0)
		{
			...
			for (n1=0; n1<movie_nodes_X1; n1++)
			{
				for (n2=0; n2<movie_nodes_X2; n2++)
				{
					...
					//do NOT try to receive data from node 0 (yourself), place available data
					{
						PlaceMagneticConductivityFrame(PropertyIndexArray,MagneticConductivityArray,flt4_MovieFrameArray);
					}
					else
					//receive data from other nodes
					{
		...
		{
			//send data to node 0
			PlaceMagneticConductivityFrame(PropertyIndexArray,MagneticConductivityArray,flt4_PartialFrameBuf_send);
			MPI_Send(flt4_PartialFrameBuf_send.data(),flt4_PartialFrameBuf_send.size(),MPI_FLOAT,0,0,MPI_CartMovieComm);
		}
#endif

#ifdef MPI_USE_PARALLEL_IO
		PlaceMagneticConductivityFrame(PropertyIndexArray,MagneticConductivityArray,flt4_PartialFrame);
		//write out the data
		//blitz++ does row-major ordering, therefore the 2nd dimension gets written first
		//use collective write to ensure that all nodes write their material properties
		MPI_File_write_all(MPIMovieFile,flt4_PartialFrame.data(),flt4_PartialFrame.size(),MPI_FLOAT,&Status);
...
// or the same size as the whole cross section (uchar1_MovieFrameArray for rank=0).
	for (x1=FrameMin_X1; x1<=FrameMax_X1; x1++)
	{
		for (x2=FrameMin_X2; x2<=FrameMax_X2; x2++)
		{
			...
			{
				field_value = sqrt(pow((Hx(xPos,yPos,zPos)+Hx(xPos+1,yPos,zPos))/2.0,2)
								+pow((Hy(xPos,yPos,zPos)+Hy(xPos,yPos+1,zPos))/2.0,2)
								+pow((Hz(xPos,yPos,zPos)+Hz(xPos,yPos,zPos+1))/2.0,2));
			}
			else
			{
...
void Crecorder_movie::PlaceFrame(const Array<float,3>* RecordedArray, Array<float,2> DestinationArray)
{//for "float" (4 byte) recording
//places the partial frame in DestinationArray, which may be the same size as the partial frame (flt4_PartialFrameBuf_send for rank!=0)
// or the same size as the whole cross section (flt4_MovieFrameArray for rank=0).
	for (x1=FrameMin_X1; x1<=FrameMax_X1; x1++)
	{
		for (x2=FrameMin_X2; x2<=FrameMax_X2; x2++)
		{
			...
			//no discretization necessary, since field value is recorded in "float" type
...
void Crecorder_movie::PlacePermittivityFrame(const Array<eps_x_type,3>* PropertyIndexArray, const Array<float,1>* PermittivityArray, Array<float,2> DestinationArray)
{
	for (x1=FrameMin_X1; x1<=FrameMax_X1; x1++)
	{
		for (x2=FrameMin_X2; x2<=FrameMax_X2; x2++)
		{
			...
			if (component=="E")
			{
				DestinationArray(x1,x2) = (eps_x(eps_x_indices(xPos,yPos,zPos))+eps_x(eps_x_indices(xPos,yPos+1,zPos))
										+eps_x(eps_x_indices(xPos,yPos,zPos+1))+eps_x(eps_x_indices(xPos,yPos+1,zPos+1))
								+eps_y(eps_y_indices(xPos,yPos,zPos))+eps_y(eps_y_indices(xPos+1,yPos,zPos))
										+eps_y(eps_y_indices(xPos,yPos,zPos+1))+eps_y(eps_y_indices(xPos+1,yPos,zPos+1))
								+eps_z(eps_z_indices(xPos,yPos,zPos))+eps_z(eps_z_indices(xPos,yPos+1,zPos))
										+eps_z(eps_z_indices(xPos+1,yPos,zPos))+eps_z(eps_z_indices(xPos+1,yPos+1,zPos)))/12.0;
...
void Crecorder_movie::PlaceElectricConductivityFrame(const Array<cond_e_x_type,3>* PropertyIndexArray, const Array<float,1>* ConductivityArray, Array<float,2> DestinationArray)
{
	for (x1=FrameMin_X1; x1<=FrameMax_X1; x1++)
	{
		for (x2=FrameMin_X2; x2<=FrameMax_X2; x2++)
		{
			...
			if (component=="E")
			{
				DestinationArray(x1,x2) = (cond_e_x(cond_e_x_indices(xPos,yPos,zPos))+cond_e_x(cond_e_x_indices(xPos,yPos+1,zPos))
										+cond_e_x(cond_e_x_indices(xPos,yPos,zPos+1))+cond_e_x(cond_e_x_indices(xPos,yPos+1,zPos+1))
								+cond_e_y(cond_e_y_indices(xPos,yPos,zPos))+cond_e_y(cond_e_y_indices(xPos+1,yPos,zPos))
										+cond_e_y(cond_e_y_indices(xPos,yPos,zPos+1))+cond_e_y(cond_e_y_indices(xPos+1,yPos,zPos+1))
								+cond_e_z(cond_e_z_indices(xPos,yPos,zPos))+cond_e_z(cond_e_z_indices(xPos,yPos+1,zPos))
										+cond_e_z(cond_e_z_indices(xPos+1,yPos,zPos))+cond_e_z(cond_e_z_indices(xPos+1,yPos+1,zPos)))/12.0;
								//doesn't really make much sense if the material is anisotropic, but that's for later
			}
			else
			{
...
void Crecorder_movie::PlacePermeabilityFrame(const Array<mu_x_type,3>* PropertyIndexArray, const Array<float,1>* PermeabilityArray, Array<float,2> DestinationArray)
{
	for (x1=FrameMin_X1; x1<=FrameMax_X1; x1++)
	{
		for (x2=FrameMin_X2; x2<=FrameMax_X2; x2++)
		{
			...
			if (component=="H")
			{
				DestinationArray(x1,x2) = (mu_x(mu_x_indices(xPos,yPos,zPos))+mu_x(mu_x_indices(xPos+1,yPos,zPos))
								+mu_y(mu_y_indices(xPos,yPos,zPos))+mu_y(mu_y_indices(xPos,yPos+1,zPos))
								+mu_z(mu_z_indices(xPos,yPos,zPos))+mu_z(mu_z_indices(xPos,yPos,zPos+1)))/6.0;
								//doesn't really make much sense if the material is anisotropic, but that's for later
			}
			else
			{
				DestinationArray(x1,x2) = (*PermeabilityArray)((*PropertyIndexArray)(xPos,yPos,zPos));
			}
		}
	}
}

void Crecorder_movie::PlaceMagneticConductivityFrame(const Array<cond_h_x_type,3>* PropertyIndexArray, const Array<float,1>* MagneticConductivityArray, Array<float,2> DestinationArray)
{
	for (x1=FrameMin_X1; x1<=FrameMax_X1; x1++)
	{
		for (x2=FrameMin_X2; x2<=FrameMax_X2; x2++)
		{
			...
			if (component=="H")
			{
				DestinationArray(x1,x2) = (cond_h_x(cond_h_x_indices(xPos,yPos,zPos))+cond_h_x(cond_h_x_indices(xPos+1,yPos,zPos))
								+cond_h_y(cond_h_y_indices(xPos,yPos,zPos))+cond_h_y(cond_h_y_indices(xPos,yPos+1,zPos))
								+cond_h_z(cond_h_z_indices(xPos,yPos,zPos))+cond_h_z(cond_h_z_indices(xPos,yPos,zPos+1)))/6.0;
								//doesn't really make much sense if the material is anisotropic, but that's for later
			}
			else
			{
				DestinationArray(x1,x2) = (*MagneticConductivityArray)((*PropertyIndexArray)(xPos,yPos,zPos));
			}
		}
	}
}
