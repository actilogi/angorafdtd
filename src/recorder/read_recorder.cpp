...
void read_recorder(Crecorder &Recorder, const Config& fdtdconfig, const Config& validsettings)
{
	...
	if (fdtdconfig.exists(recorder_setting_path))
	{
		...
		if (SettingEnabledForGrid(Recordersettings))		//apply only if enabled for this grid
		{
			...
			if (Recordersettings.exists(movie_setting_path))
			{
				...
				for (int movieindex=0; movieindex<num_of_movies; movieindex++)
				{
					...
					if (SettingEnabledForGrid(Moviesettings))		//apply only if enabled for this grid
					{
						...
						int recorded_relative_cell_index;
						double recorded_position;
						if (read_length_from_group<double>(Moviesettings,"recorded_position",recorded_position))
						{//returned true: given in meters
							recorded_relative_cell_index = (int)round(recorded_position/dx);
						}
						else
						{//returned false: given in grid cells
							recorded_relative_cell_index = (int)round(recorded_position);
						}
						//read recording position
						...
						//read path
						read_optional_value_from_group<string>(Moviesettings,"movie_dir",MovieFilePath);
						...
						moviefilenamestream << MovieFilePath << MovieFileName << "_" << recorded_component << "_"
							<< recorded_section << "_" << GridIndex;
						bool append_group_index_to_file_name;
						if (!read_optional_value_from_group<bool>(Moviesettings,"append_group_index_to_file_name",append_group_index_to_file_name))
						{
							append_group_index_to_file_name = true;
						}
						if (append_group_index_to_file_name)
						{
							moviefilenamestream << "_" << movieindex;
						...
						if (!check_mode)
						{
							Recorder.AddMovieRecorder(recorded_section,recorded_relative_cell_index,recorded_component,recording_scale,recording_type,MovieFullFileName,OnlyRecordsGeometry);
						}
					}
				}
			}
			...
			if (Recordersettings.exists(line_setting_path))
			{
				...
				for (int lineindex=0; lineindex<num_of_lines; lineindex++)
				{
					...
					if (SettingEnabledForGrid(Linesettings))		//apply only if enabled for this grid
					{
						...
						//read line positions
						int line_relative_cell_index_x1,line_relative_cell_index_x2;
						double line_position_x1,line_position_x2;
						if (read_length_from_group<double>(Linesettings,"line_position_x1",line_position_x1))
						{//returned true: given in meters
							line_relative_cell_index_x1 = (int)round(line_position_x1/dx);
						}
						else
						{//returned false: given in grid cells
							line_relative_cell_index_x1 = (int)round(line_position_x1);
						}
						if (read_length_from_group<double>(Linesettings,"line_position_x2",line_position_x2))
						{//returned true: given in meters
							line_relative_cell_index_x2 = (int)round(line_position_x2/dx);
						}
						else
						{//returned false: given in grid cells
							line_relative_cell_index_x2 = (int)round(line_position_x2);
						}
						...
						//read path
						string LineFilePath;
						read_optional_value_from_group<string>(Linesettings,"line_dir",LineFilePath);
						...
						{
							linefilenamestream << LineFilePath << LineFileName << "_" << recorded_component << "_X_" << GridIndex;
						}
						else if (line_orientation=="y_directed")
						{
							linefilenamestream << LineFilePath << LineFileName << "_" << recorded_component << "_Y_" << GridIndex;
						}
						else if (line_orientation=="z_directed")
						{
							linefilenamestream << LineFilePath << LineFileName << "_" << recorded_component << "_Z_" << GridIndex;
						}

						bool append_group_index_to_file_name;
						if (!read_optional_value_from_group<bool>(Linesettings,"append_group_index_to_file_name",append_group_index_to_file_name))
						{
							append_group_index_to_file_name = true;
						}
						if (append_group_index_to_file_name)
						{
							linefilenamestream << "_" << lineindex;
						...
						if (!check_mode)
						{
							...
							Recorder.AddLineRecorder(line_orientation,line_relative_cell_index_x1,line_relative_cell_index_x2,recorded_component,recording_scale,LineFullFileName);
						}
					}
				}
			}
			...
			if (Recordersettings.exists(fieldvalue_setting_path))
			{
				...
				for (int fieldvalueindex=0; fieldvalueindex<num_of_fieldvalues; fieldvalueindex++)
				{
					...
					if (SettingEnabledForGrid(Fieldvaluesettings))		//apply only if enabled for this grid
					{
						...
						int field_value_relative_cell_index_x,field_value_relative_cell_index_y,field_value_relative_cell_index_z;
						double coord_x,coord_y,coord_z;
						if (read_length_from_group<double>(Fieldvaluesettings,"coord_x",coord_x))
						{//returned true: given in meters
							field_value_relative_cell_index_x = (int)round(coord_x/dx);
						}
						else
						{//returned false: given in grid cells
							field_value_relative_cell_index_x = (int)round(coord_x);
						}
						if (read_length_from_group<double>(Fieldvaluesettings,"coord_y",coord_y))
						{//returned true: given in meters
							field_value_relative_cell_index_y = (int)round(coord_y/dx);
						}
						else
						{//returned false: given in grid cells
							field_value_relative_cell_index_y = (int)round(coord_y);
						}
						if (read_length_from_group<double>(Fieldvaluesettings,"coord_z",coord_z))
						{//returned true: given in meters
							field_value_relative_cell_index_z = (int)round(coord_z/dx);
						}
						else
						{//returned false: given in grid cells
							field_value_relative_cell_index_z = (int)round(coord_z);
						}
						...
						read_optional_value_from_group<string>(Fieldvaluesettings,"field_value_dir",FieldValueFilePath);
						...
						fieldvaluefilenamestream << FieldValueFilePath << FieldValueFileName << "_" << recorded_component << "_" << GridIndex;
						bool append_group_index_to_file_name;
						if (!read_optional_value_from_group<bool>(Fieldvaluesettings,"append_group_index_to_file_name",append_group_index_to_file_name))
						{
							append_group_index_to_file_name = true;
						}
						if (append_group_index_to_file_name)
						{
							fieldvaluefilenamestream << "_" << fieldvalueindex;
						...
						if (!check_mode)
						{
							...
							Recorder.AddFieldValueRecorder(field_value_relative_cell_index_x,field_value_relative_cell_index_y,field_value_relative_cell_index_z,recorded_component,recording_scale,FieldValueFullFileName);
						}
					}
				}
			}
