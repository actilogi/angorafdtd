...
//		InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
	...
	if ((component=="E")||
		...
		(component=="H")||
		((component=="Hx")&&(orientation!="x_directed"))||((component=="Hy")&&(orientation!="y_directed"))||((component=="Hz")&&(orientation!="z_directed")))
	{
		X_extra = 0;
	}
	else
...
// or the same size as the whole line (LineArray for rank=0).
	for (s=LineMin; s<=LineMax; s++)
	{
		...
		{
			field_value = Hx(xPos,yPos,zPos);
		}
		else if (component=="Hy")
		{
			field_value = Hy(xPos,yPos,zPos);
		}
		else if (component=="Hz")
		{
			field_value = Hz(xPos,yPos,zPos);
		}
