...
//			InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
	...
	if ((component!="E")&&(component!="Ex")&&(component!="Ey")&&(component!="Ez")&&(component!="H")&&(component!="Hx")&&(component!="Hy")&&(component!="Hz"))
	{
#ifdef __GNUG__
//GNU C++ compiler is being used, use the nice predefined variables for the function name
//			InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
...
{//records a single field value
	...
	if (FieldComponentInNode)
	{
		...
		{
			field_value = sqrt(pow((Hx(FieldPos_x,FieldPos_y,FieldPos_z)+Hx(FieldPos_x+1,FieldPos_y,FieldPos_z))/2.0,2)
						+pow((Hy(FieldPos_x,FieldPos_y,FieldPos_z)+Hy(FieldPos_x,FieldPos_y+1,FieldPos_z))/2.0,2)
						+pow((Hz(FieldPos_x,FieldPos_y,FieldPos_z)+Hz(FieldPos_x,FieldPos_y,FieldPos_z+1))/2.0,2));
		}
		...
		{
			field_value = Hx(FieldPos_x,FieldPos_y,FieldPos_z);
		}
		else if (component=="Hy")
		{
			field_value = Hy(FieldPos_x,FieldPos_y,FieldPos_z);
		}
		else if (component=="Hz")
		{
			field_value = Hz(FieldPos_x,FieldPos_y,FieldPos_z);
		}
