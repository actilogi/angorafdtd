...
(
			...
			const_Cshape_shared_ptr shapeptr, //shared pointer to the shape object
			const int& random_seed) //random integer seed for the random-number generator
{
/* Generates a three-dimensional (3-D) zero-mean real correlated array x[a,b,c] of dimensions AxBxC.
...
//		InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
	...
	int A_orig = BlockFront-BlockBack+1;
	int B_orig = BlockRight-BlockLeft+1;
	int C_orig = BlockUpper-BlockLower+1;
	int A,B,C; //dimensions of the generated array, which is possibly cropped into (A_origxB_origxC_orig) later

	double lc_cell = corr_len/dx;
	double std_dev_norm = std_dev/mean;

//	int max_allowable_cube_dim = 400;
	int min_req_lcs = 10;
	int min_necessary_dim = round(min_req_lcs*lc_cell);
	int limiting_dim = min(min(A_orig,B_orig),C_orig);
	if (limiting_dim>min_necessary_dim)
	{//if limiting_dim>min_necessary_dim, no expansion necessary
		A=A_orig;
		B=B_orig;
		C=C_orig;
	}
	else
	{//if limiting_dim<min_necessary_dim, generate bigger sample and take a subset of it
		if (A_orig<min_necessary_dim)
			A=min_necessary_dim;
		else
			A=A_orig;
		if (B_orig<min_necessary_dim)
			B=min_necessary_dim;
		else
			B=B_orig;
		if (C_orig<min_necessary_dim)
			C=min_necessary_dim;
		else
			C=C_orig;
//		if ((A_orig*N_orig*P_orig)>max_allowable_cube_dim^3)
//		{
//			warning(['Memory limit reached. Statistical averaging may not be adequate. (Array size: '...
//				num2str(A_orig*N_orig*P_orig) ', Available size: ' num2str(max_allowable_cube_dim^3) ')']);
//		}
	}
...
//		fwrite(&C, sizeof(C), 1, randfilestream);
		//dimensions of the possibly cropped array
		fwrite(&A_orig, sizeof(A_orig), 1, randfilestream);
		fwrite(&B_orig, sizeof(B_orig), 1, randfilestream);
		fwrite(&C_orig, sizeof(C_orig), 1, randfilestream);
...
//exit(-1);*/
		...
		//A,B,C are the dimensions of the possibly extended array, which might be cropped later to the desired size
...
///** REMOVE LATER !!!! **/
///** REMOVE LATER !!!! **/
//char randfile_template2[] = "./fdtd-XXXXXX";
//FILE * randfilestream2;
//int randfiledes2 = mkstemp(randfile_template2); /** GNU-specific **/ //creates temporary file, returns low-level file descriptor (note that "file descriptors" are GNU-specific)
//randfilestream2 = fdopen(randfiledes2,"w+"); /** GNU-specific **/ //convert file descriptor to higher-level stream, open stream for write/read {int i,j,k;
//fwrite(&A, sizeof(A), 1, randfilestream2);
//fwrite(&B, sizeof(B), 1, randfilestream2);
//fwrite(&C, sizeof(C), 1, randfilestream2);
//{int i,j,k;
...
//for (k=0; k<C; k++) //only write the necessary section (of length C<=C)
//{
//	for (j=0; j<B; j++) //only write the necessary section (of length B<=B)
//	{
...
//		fwrite(real_rand_column.data(),sizeof(RandMatType),A,randfilestream2); //only write the necessary section (of length A<=A)
//	}
//}
//}
//fclose(randfilestream2);
//string randfilename2 = randfile_template2; //copy the filename into string (needed for PlaceMaterialRegionFromFile)
//int a;
//cout << "File name is " << randfilename2 << endl;
//cin >> a;
//unlink(randfilename2.c_str());
///** REMOVE LATER !!!! **/
///** REMOVE LATER !!!! **/
		...
		Array<RandMatType,1> real_rand_column(A); //the actual dimension of the random array will be A
		Array<complex<RandMatType>,1> this_column(even_A/2+1),conjugate_column(even_A/2+1); //temporary complex columns from which the real data is extracted (these two are symmetrically positioned w.r.t. the center)
		for (k=0; k<C_orig; k++) //only write the necessary section (of length C_orig<=C)
		{
			for (j=0; j<B_orig; j++) //only write the necessary section (of length B_orig<=B)
			{
				this_column = rand_output(k,j,Range::all());
				conjugate_column = rand_output((C-k)%C,(B-j)%B,Range::all()); //symmetric position
//cout << this_column << endl;
				...
				if (A>=3)
				real_rand_column(Range(even_A/2+1,A-1)) = real(conjugate_column(Range(even_A/2-1,even_A-A+1,-1)))-imag(conjugate_column(Range(even_A/2-1,even_A-A+1,-1)));
				fwrite(real_rand_column.data(),sizeof(RandMatType),A_orig,randfilestream); //only write the necessary section (of length A_orig<=A)
			}
		}
		}
//
...
template <typename RandMatType>
void PlaceWMCorrRandomBlock
(const string& constitutive_param_type, const double& mean, const double& std_dev, const double& corr_len, const double& m, const_Cshape_shared_ptr shapeptr)
{//the version of PlaceWMCorrRandomBlock that creates its own random seed from the system time
	...
	PlaceWMCorrRandomBlock<RandMatType>(constitutive_param_type,mean,std_dev,corr_len,m,shapeptr,random_seed);
}
