...
void read_geom(const Config& fdtdconfig, const Config& validsettings, const Cshapes &Shapes, const Cmats& Materials)
{
	...
	if (fdtdconfig.exists(geometry_setting_path))
	{
		...
		if (SettingEnabledForGrid(Geometrysettings))		//apply only if enabled for this grid
		{
			...
			for (int geometry_setting_index=0; geometry_setting_index<num_of_geometry_settings; geometry_setting_index++)
			{
				...
				if (geometry_setting_name=="Objects")
				{
					...
					for (int object_setting_index=0; object_setting_index<num_of_object_settings; object_setting_index++)
					{
						...
						if (SettingEnabledForGrid(Objectsetting))		//apply only if enabled for this grid
						{
							//read the shape
							string shape_tag;
							read_value_from_group<string>(Objectsetting,"shape_tag",shape_tag);
							const_Cshape_shared_ptr shapeptr = Shapes[shape_tag];

							//read the material
							string material_tag;
							read_value_from_group<string>(Objectsetting,"material_tag",material_tag);
							Cmat material = Materials[material_tag];

							if (!check_mode)
							{
								//place the object into the grid
								place_obj(material,shapeptr);
							}
						}
					}
				}//if exists
				...
				else if (geometry_setting_name=="MaterialSlabs")
				{
					...
					for (int slabindex=0; slabindex<num_of_slabs; slabindex++)
					{
						...
						catch (AngoraInvalidSettingNameException& excp)
						{
							...
							{
								throw AngoraSettingDeprecatedException(Slabsettings["start_position"],"Use min_coord_in_cells instead. (Conversion hint: if start_position is a, min_coord_in_cells is a-1)");
							}
							else if (excp.getSettingName()=="end_position")
							{
								throw AngoraSettingDeprecatedException(Slabsettings["end_position"],"Use max_coord_in_cells instead. (Conversion hint: if end_position is a, max_coord_in_cells is also a)");
							}
							else
						...
						{
							string tag;
							read_value_from_group<string>(Slabsettings,"material_tag",tag);

							Cmat material = Materials[tag];

							//z coordinates of the lower and upper interfaces w.r.t. the origin point (can be non-integer)
							double min_coord;
							bool given_in_m;
							try{
								given_in_m = read_length_from_group<double>(Slabsettings,"min_coord",min_coord);
								if (given_in_m)
								{//given in meters: convert to grid cells
									min_coord /= dx;
								}
							...
							{//it could be string
								...
								try{
									given_in_m = read_length_from_group<string>(Slabsettings,"min_coord",min_coord_str);
									...
									else
									{
										if (given_in_m)
											...
											throw AngoraInvalidSettingValueException(Slabsettings["min_coord_in_cells"],"only allowable string value is \"min\"");
									}
								}
								catch (AngoraInvalidSettingTypeException& exc)
								{//if it is neither numeric nor string, the exception is caught here
									throw AngoraInvalidSettingTypeException(exc.getSetting(),"should be a numeric value or the string \"min\"");
								}
							}
							double max_coord;
							try{
								given_in_m = read_length_from_group<double>(Slabsettings,"max_coord",max_coord);
								if (given_in_m)
								{//given in meters: convert to grid cells
									max_coord /= dx;
								}
							...
							{//it could be string
								...
								try{
									given_in_m = read_length_from_group<string>(Slabsettings,"max_coord",max_coord_str);
									...
									else
									{
										if (given_in_m)
											...
											throw AngoraInvalidSettingValueException(Slabsettings["max_coord_in_cells"],"only allowable string value is \"max\"");
									}
								}
								catch (AngoraInvalidSettingTypeException& exc)
								{//if it is neither numeric nor string, the exception is caught here
									throw AngoraInvalidSettingTypeException(exc.getSetting(),"should be a numeric value or the string \"max\"");
								}
							}
							//check the slab bounds
							if (max_coord<=min_coord)
							{
								if (given_in_m)
									throw AngoraInvalidSettingValueException(Slabsettings["max_coord"],"thickness of slab should be positive");
								else
									throw AngoraInvalidSettingValueException(Slabsettings["max_coord_in_cells"],"thickness of slab should be positive");
//								throw AngoraInvalidSettingValueException(Slabsettings,"\"max_coord\"cannot be smaller than \"min_coord\"");
							}

							//If not in check mode, place the slab in grid
							if (!check_mode)
							{
								PlaceSlab(material,min_coord,max_coord);
							}
						}
					}
				}
	...
	/***********************************/
				else if (geometry_setting_name=="GroundPlanes")
				{
					...
					for (int groundplaneindex=0; groundplaneindex<num_of_groundplanes; groundplaneindex++)
					{
						...
						catch (AngoraInvalidSettingNameException& excp)
						{
							...
							{
								throw AngoraSettingDeprecatedException(Groundplanesettings["position_z"],"Use \"coord_in_cells\" instead.");
							}
							else
							{//rethrow the invalid setting exception
								throw;
						...
						if (SettingEnabledForGrid(Groundplanesettings))		//apply only if enabled for this grid
						{
							...
							int coord_in_cells;
							double coord;
							if (read_length_from_group<double>(Groundplanesettings,"coord",coord))
							{//returned true: given in meters
								coord_in_cells = (int)round(coord/dx)+(OriginZ-1); //relative to grid origin
							}
							else
							{//returned false: given in grid cells
								coord_in_cells = (int)round(coord)+(OriginZ-1); //relative to grid origin
							}

							//If not in check mode, place the slab in grid
							if (!check_mode)
							{
								PlaceGround(coord_in_cells);
							}
						}
					}
				}
				else if (geometry_setting_name=="RandomMaterials")
				{
					...
					if (SettingEnabledForGrid(Randommaterialsettings))		//apply only if enabled for this grid
					{
						//treat the settings in order of appearance in the config file
							...
							if (random_material_setting_name=="WhittleMaternCorrelated")
							{
								...
								for (int wmindex=0; wmindex<num_of_wms; wmindex++)
								{
									...
									if (SettingEnabledForGrid(WMblocksettings))		//apply only if enabled for this grid
									{
										...
										if (!read_length_from_group<double>(WMblocksettings,"corr_len",corr_len))
										{//returned false: given in grid cells
											corr_len *= dx;
										}
		...
		//								if (read_length_from_group<double>(WMblocksettings,"back_coord_x",back_coord))
		//								{//returned true: given in meters
		//									back_coord /= dx;
		//								}
		//								if (read_length_from_group<double>(WMblocksettings,"front_coord_x",front_coord))
		//								{//returned true: given in meters
		//									front_coord /= dx;
		//								}
		//								if (read_length_from_group<double>(WMblocksettings,"left_coord_y",left_coord))
		//								{//returned true: given in meters
		//									left_coord /= dx;
		//								}
		//								if (read_length_from_group<double>(WMblocksettings,"right_coord_y",right_coord))
		//								{//returned true: given in meters
		//									right_coord /= dx;
		//								}
		//								if (read_length_from_group<double>(WMblocksettings,"lower_coord_z",lower_coord))
		//								{//returned true: given in meters
		//									lower_coord /= dx;
		//								}
		//								if (read_length_from_group<double>(WMblocksettings,"upper_coord_z",upper_coord))
		//								{//returned true: given in meters
		//									upper_coord /= dx;
		//								}
		//
		...
		//								upper_coord += OriginZ-1;
										...
										//read the shape
										string shape_tag;
										read_value_from_group<string>(WMblocksettings,"shape_tag",shape_tag);
										const_Cshape_shared_ptr shapeptr = Shapes[shape_tag];
										...
										if (!check_mode)
										{
											...
											if (!random_seed_exists)
											{
												PlaceWMCorrRandomBlock<float>(constitutive_param_type,mean,std_dev,corr_len,m,shapeptr);
											}
											else
											{
												PlaceWMCorrRandomBlock<float>(constitutive_param_type,mean,std_dev,corr_len,m,shapeptr,random_seed);
											}
											/** More flexible later? **/
										}
									}
				...
				else if (geometry_setting_name=="MaterialsFromFiles")
				{
					...
					for (int matfileindex=0; matfileindex<num_of_matfiles; matfileindex++)
					{
						...
						if (SettingEnabledForGrid(Matfilesettings))		//apply only if enabled for this grid
						{
							...
							if (read_length_from_group<double>(Matfilesettings,"coord_x",coord_x))
							{//returned true: given in meters
								position_x = (int)round(coord_x/dx)+(OriginX-1); //relative to grid origin
							}
							else
							{//returned false: given in grid cells
								position_x = (int)round(coord_x)+(OriginX-1); //relative to grid origin
							}
							if (read_length_from_group<double>(Matfilesettings,"coord_y",coord_y))
							{//returned true: given in meters
								position_y = (int)round(coord_y/dx)+(OriginY-1); //relative to grid origin
							}
							else
							{//returned false: given in grid cells
								position_y = (int)round(coord_y)+(OriginY-1); //relative to grid origin
							}
							if (read_length_from_group<double>(Matfilesettings,"coord_z",coord_z))
							{//returned true: given in meters
								position_z = (int)round(coord_z/dx)+(OriginZ-1); //relative to grid origin
							}
							else
							{//returned false: given in grid cells
								position_z = (int)round(coord_z)+(OriginZ-1); //relative to grid origin
							}
