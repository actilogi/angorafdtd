...
//		dataset.write(&L,PredType::NATIVE_INT);
		...
		/** write the plane-wave information **/
		...
		if ((Data.write_pw_info)&&(num_of_PWs>0))
		{//don't write if there are no plane waves (num_of_PWs=0), otherwise PW_THETA(0) below will give runtime error
			...
			/** write the scattered PW phasors **/
			//create temporary arrays to hold the 2D data [less elegant, but more portable than creating strided dataspaces]
			Array<double,2> temp2D_1(num_of_PWs,L),temp2D_2(num_of_PWs,L);
			//dataspace for the 2D arrays
			hsize_t pwphasordims[] = {num_of_PWs,L};
			DataSpace dspace2D( 2, pwphasordims);
			for (int l=0; l<L; l++)
			{
				for (int PW=0; PW<num_of_PWs; PW++)	//could have also used "PW_PHI.size()", since they are supposed to be of the same size (num. of PWs)
				{
