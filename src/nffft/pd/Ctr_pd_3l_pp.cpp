...
//Defines the post-processing steps in the PHASOR-DOMAIN near-field-to-far-field transformer object "Ctr_pd_3l" in a 3-layered medium
...
void Ctr_pd_3l::ConstructPotential_A(const int& l, const int& d1, const int& d2)
{
	...
	if (Back_Hz_PassesThroughNode)
	{
		...
		{
			for (k=max(MiddleLayerPos+1,Hz_lowerlimit_in_node);k<=Hz_upperlimit_in_node;k++)
			{//upper half space
				z_pr = k-(MiddleLayerPos+1);
			...
			for (k=max(Hz_lowerlimit_in_node,MiddleLayerPos-MiddleLayerThickness+1);k<=min(MiddleLayerPos,Hz_upperlimit_in_node);k++)
			{//middle layer
				z_pr = k-(MiddleLayerPos+1);
				A_theta(l,d1,d2) += J_y_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
			...
			for (k=Hz_lowerlimit_in_node;k<=min(MiddleLayerPos-MiddleLayerThickness,Hz_upperlimit_in_node);k++)
			{//lower half space
				z_pr = k-(MiddleLayerPos+1);
				A_theta(l,d1,d2) += J_y_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
						*(exp(ii*kt2_dx*z_pr)*Green_A_theta_J_y_upward_2
						 +exp(-ii*kt2_dx*z_pr)*Green_A_theta_J_y_downward_2);
				A_phi(l,d1,d2) += J_y_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
						*(exp(ii*kt2_dx*z_pr)*Green_A_phi_J_y_upward_2
						 +exp(-ii*kt2_dx*z_pr)*Green_A_phi_J_y_downward_2);
			}
		}
	}
	...
	if (Back_Hy_PassesThroughNode)
	{
		...
		{
			for (k=max(MiddleLayerPos+1,Hy_lowerlimit_in_node);k<=Hy_upperlimit_in_node;k++)
			{//upper half space
				z_pr = k-(MiddleLayerPos+1)+0.5;
			...
			for (k=max(Hy_lowerlimit_in_node,MiddleLayerPos-MiddleLayerThickness+1);k<=min(MiddleLayerPos,Hy_upperlimit_in_node);k++)
			{//middle layer
				z_pr = k-(MiddleLayerPos+1)+0.5;
				A_theta(l,d1,d2) += J_z_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
			...
			for (k=Hy_lowerlimit_in_node;k<=min(MiddleLayerPos-MiddleLayerThickness,Hy_upperlimit_in_node);k++)
			{//lower half space
				z_pr = k-(MiddleLayerPos+1)+0.5;
				A_theta(l,d1,d2) += J_z_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
						*(exp(ii*kt2_dx*z_pr)*Green_A_theta_J_z_upward_2
						 +exp(-ii*kt2_dx*z_pr)*Green_A_theta_J_z_downward_2);
			}
		}
	}
	...
	if (Left_Hz_PassesThroughNode)
	{
		...
		for (i=Hz_backlimit_in_node;i<=Hz_frontlimit_in_node;i++)
		{
			...
			{//lower half space
				...
				A_theta(l,d1,d2) += J_x_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
						*(exp(ii*kt2_dx*z_pr)*Green_A_theta_J_x_upward_2
						 +exp(-ii*kt2_dx*z_pr)*Green_A_theta_J_x_downward_2);
				A_phi(l,d1,d2) += J_x_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
						*(exp(ii*kt2_dx*z_pr)*Green_A_phi_J_x_upward_2
						 +exp(-ii*kt2_dx*z_pr)*Green_A_phi_J_x_downward_2);
			}
		}
	}
	...
	if (Left_Hx_PassesThroughNode)
	{
		...
		{
			for (k=max(MiddleLayerPos+1,Hx_lowerlimit_in_node);k<=Hx_upperlimit_in_node;k++)
			{//upper half space
				z_pr = k-(MiddleLayerPos+1)+0.5;
			...
			for (k=max(Hx_lowerlimit_in_node,MiddleLayerPos-MiddleLayerThickness+1);k<=min(MiddleLayerPos,Hx_upperlimit_in_node);k++)
			{//middle layer
				z_pr = k-(MiddleLayerPos+1)+0.5;
				A_theta(l,d1,d2) += J_z_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
			...
			for (k=Hx_lowerlimit_in_node;k<=min(MiddleLayerPos-MiddleLayerThickness,Hx_upperlimit_in_node);k++)
			{//lower half space
				z_pr = k-(MiddleLayerPos+1)+0.5;
				A_theta(l,d1,d2) += J_z_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
	...
	if (Lower_Hy_PassesThroughNode)
	{
		...
		else if ((k>(MiddleLayerPos-MiddleLayerThickness))&&(k<=MiddleLayerPos))
		{//middle layer
			for (i=Hy_backlimit_in_node;i<=Hy_frontlimit_in_node;i++)
			{
				for (j=Hy_leftlimit_in_node;j<=Hy_rightlimit_in_node;j++)
...
void Ctr_pd_3l::ConstructPotential_F(const int& l, const int& d1, const int& d2)
{
	...
	if (Back_Ez_PassesThroughNode)
	{
		...
		{
			for (k=max(MiddleLayerPos+1,Ez_lowerlimit_in_node);k<=Ez_upperlimit_in_node;k++)
			{//upper half space
				z_pr = k-(MiddleLayerPos+1)+0.5;
			...
			for (k=max(Ez_lowerlimit_in_node,MiddleLayerPos-MiddleLayerThickness+1);k<=min(MiddleLayerPos,Ez_upperlimit_in_node);k++)
			{//middle layer
				z_pr = k-(MiddleLayerPos+1)+0.5;
				F_theta(l,d1,d2) += M_y_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
			...
			for (k=Ez_lowerlimit_in_node;k<=min(MiddleLayerPos-MiddleLayerThickness,Ez_upperlimit_in_node);k++)
			{//lower half space
				z_pr = k-(MiddleLayerPos+1)+0.5;
				F_theta(l,d1,d2) += M_y_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
							*(exp(ii*kt2_dx*z_pr)*Green_F_theta_M_y_upward_2
						 	 +exp(-ii*kt2_dx*z_pr)*Green_F_theta_M_y_downward_2);
				F_phi(l,d1,d2) += M_y_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY)*sinTsinP))
							*(exp(ii*kt2_dx*z_pr)*Green_F_phi_M_y_upward_2
						 	 +exp(-ii*kt2_dx*z_pr)*Green_F_phi_M_y_downward_2);
			}
		}
	}
	...
	if (Back_Ey_PassesThroughNode)
	{
		...
		{
			for (k=max(MiddleLayerPos+1,Ey_lowerlimit_in_node);k<=Ey_upperlimit_in_node;k++)
			{//upper half space
				z_pr = k-(MiddleLayerPos+1);
			...
			for (k=max(Ey_lowerlimit_in_node,MiddleLayerPos-MiddleLayerThickness+1);k<=min(MiddleLayerPos,Ey_upperlimit_in_node);k++)
			{//middle layer
				z_pr = k-(MiddleLayerPos+1);
				F_theta(l,d1,d2) += M_z_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
			...
			for (k=Ey_lowerlimit_in_node;k<=min(MiddleLayerPos-MiddleLayerThickness,Ey_upperlimit_in_node);k++)
			{//lower half space
				z_pr = k-(MiddleLayerPos+1);
				F_theta(l,d1,d2) += M_z_back(l,j,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
							*(exp(ii*kt2_dx*z_pr)*Green_F_theta_M_z_upward_2
						 	 +exp(-ii*kt2_dx*z_pr)*Green_F_theta_M_z_downward_2);
			}
		}
	}
	...
	if (Left_Ez_PassesThroughNode)
	{
		...
		for (i=Ez_backlimit_in_node;i<=Ez_frontlimit_in_node;i++)
		{
			...
			{//lower half space
				...
				F_theta(l,d1,d2) += M_x_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
							*(exp(ii*kt2_dx*z_pr)*Green_F_theta_M_x_upward_2
						 	 +exp(-ii*kt2_dx*z_pr)*Green_F_theta_M_x_downward_2);
				F_phi(l,d1,d2) += M_x_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
							*(exp(ii*kt2_dx*z_pr)*Green_F_phi_M_x_upward_2
						 	 +exp(-ii*kt2_dx*z_pr)*Green_F_phi_M_x_downward_2);
			}
		}
	}
	...
	if (Left_Ex_PassesThroughNode)
	{
		...
		{
			for (k=max(MiddleLayerPos+1,Ex_lowerlimit_in_node);k<=Ex_upperlimit_in_node;k++)
			{//upper half space
				z_pr = k-(MiddleLayerPos+1);
			...
			for (k=max(Ex_lowerlimit_in_node,MiddleLayerPos-MiddleLayerThickness+1);k<=min(MiddleLayerPos,Ex_upperlimit_in_node);k++)
			{//middle layer
				z_pr = k-(MiddleLayerPos+1);
				F_theta(l,d1,d2) += M_z_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
			...
			for (k=Ex_lowerlimit_in_node;k<=min(MiddleLayerPos-MiddleLayerThickness,Ex_upperlimit_in_node);k++)
			{//lower half space
				z_pr = k-(MiddleLayerPos+1);
				F_theta(l,d1,d2) += M_z_left(l,i,k)
						*exp(ii*ko_dx*((i-FarFieldOriginX+0.5)*sinTcosP+(j-FarFieldOriginY+0.5)*sinTsinP))
...
complex<double> Ctr_pd_3l::TheoreticalFarFieldTheta(const int& l, const int& d1, const int& d2)
	...
	if (Data.PointSourcesPtr!=NULL)	//calculate far-field if the pointer to the Cpointsource object is defined
	{
		for (int i=0; i<Data.PointSourcesPtr->NumberOfElectricDipoles(); i++)
		{
			...
			if (Orientation=="x_directed")
			{
				...
				SourceZ = Data.PointSourcesPtr->ElectricDipole(i)->z-(MiddleLayerPos+1);
			}
			...
			else if (Orientation=="z_directed")
			{
				SourceX = Data.PointSourcesPtr->ElectricDipole(i)->x-FarFieldOriginX;
				SourceY = Data.PointSourcesPtr->ElectricDipole(i)->y-FarFieldOriginY;
				SourceZ = Data.PointSourcesPtr->ElectricDipole(i)->z-(MiddleLayerPos+1)+0.5;
			}
			...
			else if ((SourceZ>(-MiddleLayerThickness))&&(SourceZ<0))
			{//if dipole is in the middle layer
				if (Orientation=="x_directed")
				{
					theta_component *= exp(ii*ko_dx*(SourceX*sinTcosP+SourceY*sinTsinP))
			...
			{//if dipole is in lower half space
				...
				{
					theta_component *= exp(ii*ko_dx*(SourceX*sinTcosP+SourceY*sinTsinP))
							*(exp(ii*kt2_dx*SourceZ)*Green_A_theta_J_x_upward_2
						 	 +exp(-ii*kt2_dx*SourceZ)*Green_A_theta_J_x_downward_2);
				}
				else if (Orientation=="y_directed")
				{
					theta_component *= exp(ii*ko_dx*(SourceX*sinTcosP+SourceY*sinTsinP))
							*(exp(ii*kt2_dx*SourceZ)*Green_A_theta_J_y_upward_2
						 	 +exp(-ii*kt2_dx*SourceZ)*Green_A_theta_J_y_downward_2);
				}
				else if (Orientation=="z_directed")
				{
					theta_component *= exp(ii*ko_dx*(SourceX*sinTcosP+SourceY*sinTsinP))
							*(exp(ii*kt2_dx*SourceZ)*Green_A_theta_J_z_upward_2
						 	 +exp(-ii*kt2_dx*SourceZ)*Green_A_theta_J_z_downward_2);
				}
			}
...
complex<double> Ctr_pd_3l::TheoreticalFarFieldPhi(const int& l, const int& d1, const int& d2)
	...
	if (Data.PointSourcesPtr!=NULL)	//calculate far-field if the pointer to the Cpointsource object is defined
	{
		for (int i=0; i<Data.PointSourcesPtr->NumberOfElectricDipoles(); i++)
		{
			...
			{//if dipole is in lower half space
				...
				{
					phi_component *= exp(ii*ko_dx*(SourceX*sinTcosP+SourceY*sinTsinP))
							*(exp(ii*kt2_dx*SourceZ)*Green_A_phi_J_x_upward_2
						 	 +exp(-ii*kt2_dx*SourceZ)*Green_A_phi_J_x_downward_2);
				}
				else if (Orientation=="y_directed")
				{
					phi_component *= exp(ii*ko_dx*(SourceX*sinTcosP+SourceY*sinTsinP))
							*(exp(ii*kt2_dx*SourceZ)*Green_A_phi_J_y_upward_2
						 	 +exp(-ii*kt2_dx*SourceZ)*Green_A_phi_J_y_downward_2);
				}
				else if (Orientation=="z_directed")
				{
					phi_component *= 0;	//z-directed dipole does not contribute to the phi-component of the radiated E-field
