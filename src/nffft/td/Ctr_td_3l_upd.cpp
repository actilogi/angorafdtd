...
{//Update the far-field waveforms due to Mx
	delay_lateral = (r_offset_over_dx-((i-FarFieldOriginX)*SinTCosP+(j-FarFieldOriginY+0.5)*SinTSinP))*dx_over_c_o_over_dt;	//Lateral delay
...
{//Update the far-field waveforms due to My
	delay_lateral = (r_offset_over_dx-((i-FarFieldOriginX+0.5)*SinTCosP+(j-FarFieldOriginY)*SinTSinP))*dx_over_c_o_over_dt;	//Lateral delay
	//For TM waveform V_e_v:
	for (impulse=0;impulse<V_e_v(k).size();impulse++)
	{
		delay_total = delay_lateral + V_e_v(k)(impulse).Delay;
...
{//Update the far-field waveforms due to Mz
	delay_lateral = (r_offset_over_dx-((i-FarFieldOriginX+0.5)*SinTCosP+(j-FarFieldOriginY+0.5)*SinTSinP))*dx_over_c_o_over_dt;	//Lateral delay
...
{//Update the far-field waveforms due to Jz
	delay_lateral = (r_offset_over_dx-((i-FarFieldOriginX)*SinTCosP+(j-FarFieldOriginY)*SinTSinP))*dx_over_c_o_over_dt;	//Lateral delay
	//For TM waveform V_e_v:
	for (impulse=0;impulse<V_e_v(k).size();impulse++)
	{
		delay_total = delay_lateral + V_e_v(k)(impulse).Delay;
		a = delay_total-int(delay_total);
		Jz_e = epsilon_o/(eps_z(eps_z_indices_on_z_axis(k)))*V_e_v(k)(impulse).Amp*Jz;
