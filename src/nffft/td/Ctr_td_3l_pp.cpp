...
//		FarFieldFile.write((char*)&angora_version_revision,sizeof(angora_version_revision));
		...
		dataset=far_field_file.createDataSet("time_step", PredType::NATIVE_DOUBLE, dspace, plist);
		dataset.write(&dt,PredType::NATIVE_DOUBLE);
		//write initial time value
		double initial_time_value = get_initial_time_value();
		dataset=far_field_file.createDataSet("initial_time_value", PredType::NATIVE_DOUBLE, dspace, plist);
		dataset.write(&initial_time_value,PredType::NATIVE_DOUBLE);
		//write the angles
		dataset=far_field_file.createDataSet("theta", PredType::NATIVE_DOUBLE, dspace, plist);
		dataset.write(&THETA,PredType::NATIVE_DOUBLE);
		dataset=far_field_file.createDataSet("phi", PredType::NATIVE_DOUBLE, dspace, plist);
		dataset.write(&PHI,PredType::NATIVE_DOUBLE);

		//create temporary arrays to hold the time-domain data
		Array<double,1> temp1D(TotalSteps);
		//dataspace for the 1D arrays
		hsize_t far_field_array_size = TotalSteps;
		DataSpace dspace1D(1,&far_field_array_size);
		//theta component
		for (int n=0; n<TotalSteps; n++)
		{
			temp1D(n) = waveformTheta(n);
		}
		dataset=far_field_file.createDataSet("E_theta", PredType::NATIVE_DOUBLE, dspace1D, plist);
		dataset.write(temp1D.data(),PredType::NATIVE_DOUBLE);
		//phi component
		for (int n=0; n<TotalSteps; n++)
		{
			temp1D(n) = waveformPhi(n);
		}
		dataset=far_field_file.createDataSet("E_phi", PredType::NATIVE_DOUBLE, dspace1D, plist);
		dataset.write(temp1D.data(),PredType::NATIVE_DOUBLE);

		if (Data.write_hertzian_dipole_far_field)
		{
			//theoretical theta component
			for (int n=0; n<TotalSteps; n++)
			{
				temp1D(n) = TheoreticalFarFieldTheta(n);
			}
			dataset=far_field_file.createDataSet("E_theta_th", PredType::NATIVE_DOUBLE, dspace1D, plist);
			dataset.write(temp1D.data(),PredType::NATIVE_DOUBLE);
			//theoretical phi component
			for (int n=0; n<TotalSteps; n++)
			{
				temp1D(n) = TheoreticalFarFieldPhi(n);
			}
			dataset=far_field_file.createDataSet("E_phi_th", PredType::NATIVE_DOUBLE, dspace1D, plist);
			dataset.write(temp1D.data(),PredType::NATIVE_DOUBLE);
		}
	}
}
...
// 					temp += V_e_v(z)(impulse).Amp*TheoreticalFarFieldWaveform((n-delay_total)*dt-delay*tau,tau);
				...
				theta_component += (1/(2*M_PI*c_o))*(Z_o*SinT/(eps_z(eps_z_indices_on_z_axis(z))/epsilon_o))*temp; //for z-directed dipole
			}
		}
	}
