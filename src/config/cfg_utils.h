...
public:
               ...
               || (LIBCONFIG_VER_MAJOR > 1))
            /* use the getSourceFile() feature in libconfig 1.4 and later */
  AngoraInvalidSettingNameException(const Setting& invalid_setting) :_setting(invalid_setting), _setting_path(invalid_setting.getPath()), _setting_name(invalid_setting.getName()), AngoraSettingException(invalid_setting.getSourceLine(),invalid_setting.getSourceFile()) {};
#else
  AngoraInvalidSettingNameException(const Setting& invalid_setting) :_setting(invalid_setting), _setting_path(invalid_setting.getPath()), _setting_name(invalid_setting.getName()), AngoraSettingException(invalid_setting.getSourceLine(),config_filename) {};
#endif
...
public:
               ...
               || (LIBCONFIG_VER_MAJOR > 1))
            /* use the getSourceFile() feature in libconfig 1.4 and later */
  AngoraInvalidSettingTypeException(const Setting& invalid_setting, const string& allowed_types) :_setting(invalid_setting), _setting_path(invalid_setting.getPath()), _allowed_types(allowed_types), AngoraSettingException(invalid_setting.getSourceLine(),invalid_setting.getSourceFile()) {};
#else
  AngoraInvalidSettingTypeException(const Setting& invalid_setting, const string& allowed_types) :_setting(invalid_setting), _setting_path(invalid_setting.getPath()), _allowed_types(allowed_types), AngoraSettingException(invalid_setting.getSourceLine(),config_filename) {};
#endif
...
class AngoraUnnecessarySettingNameException: public AngoraSettingException
{// exception for an unknown setting
public:
#if (((LIBCONFIG_VER_MAJOR == 1) && (LIBCONFIG_VER_MINOR >= 4)) \
               || (LIBCONFIG_VER_MAJOR > 1))
            /* use the getSourceFile() feature in libconfig 1.4 and later */
  AngoraUnnecessarySettingNameException(const Setting& unnecessary_setting) :_setting_path(unnecessary_setting.getPath()), _setting_name(unnecessary_setting.getName()), AngoraSettingException(unnecessary_setting.getSourceLine(),unnecessary_setting.getSourceFile()) {};
#else
  AngoraUnnecessarySettingNameException(const Setting& unnecessary_setting) :_setting_path(unnecessary_setting.getPath()), _setting_name(unnecessary_setting.getName()), AngoraSettingException(unnecessary_setting.getSourceLine(),config_filename) {};
#endif
  virtual ~AngoraUnnecessarySettingNameException() throw() {};

  virtual const string getError() const
  {//error message
	ostringstream _msgstr;
	_msgstr << getFileName() << ": line " << getLineNumber() << ": error: " << _setting_path << " is an unnecessary setting";
	return _msgstr.str();
  };
...
template<typename SettingType>
bool read_length_from_group(const Setting& parent_group, const string& setting_name, SettingType& setting_variable)
{//reads a length variable with an alternative name appended by "_in_cells"
	if ((parent_group.exists(setting_name))&&(parent_group.exists(setting_name+"_in_cells")))
	{
		throw AngoraUnnecessarySettingNameException(parent_group[setting_name+"_in_cells"]);
	}
	if (read_optional_value_from_group<SettingType>(parent_group,setting_name,setting_variable))
	{
		return true;  //returns true if the length is given in meters (without the "_in_cells" suffix)
	}
	else
	{
		if (read_optional_value_from_group<SettingType>(parent_group,setting_name+"_in_cells",setting_variable))
		{
			return false;  //returns false if the length is given in grid cells (with the "_in_cells" suffix)
		}
		else
		{
			throw AngoraSettingNotFoundException(setting_name+" or "+setting_name+"_in_cells",parent_group);
		}
	}
}

template<typename SettingType>
bool read_optional_length_from_group(const Setting& parent_group, const string& setting_name, SettingType& setting_variable, bool& given_in_meters)
{//reads a length variable with an alternative name appended by "_in_cells"
 //if neither of them exists, returns false
 //if the length is given in meters, assigns "true" to the output var. "given_in_meters"
	if ((parent_group.exists(setting_name))&&(parent_group.exists(setting_name+"_in_cells")))
	{
		throw AngoraUnnecessarySettingNameException(parent_group[setting_name+"_in_cells"]);
	}
	if (read_optional_value_from_group<SettingType>(parent_group,setting_name,setting_variable))
	{
		given_in_meters = true; //if the length is given in meters (without the "_in_cells" suffix)
		return true; //length variable found, so return true
	}
	else
	{
		if (read_optional_value_from_group<SettingType>(parent_group,setting_name+"_in_cells",setting_variable))
		{
			given_in_meters = false; //if the length is given in grid cells (with the "_in_cells" suffix)
			return true; //length variable found, so return true
		}
		else
		{
			return false; //neither of them exists, return false
		}
	}
}
