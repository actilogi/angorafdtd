...
//		if (((this_setting_name!="enabled_for_runs")||mygroup.isRoot())&&(!group_with_valid_settings.exists(this_setting_name)))
		if ((((this_setting_name!="enabled_for_runs")&&(this_setting_name!="enabled_for_run_range"))||mygroup.isRoot())&&(!group_with_valid_settings.exists(this_setting_name)))
		{
...
{//determines if the setting "mySetting" is enabled for the current grid
	bool enabled_for_this_grid = true;
	if (mySetting.exists("enabled_for_runs"))
	{
		enabled_for_this_grid = false;
		Setting& EnabledGrids = mySetting["enabled_for_runs"];
		if (EnabledGrids.getType()!=Setting::TypeArray)
		{
			throw AngoraInvalidSettingTypeException(EnabledGrids,"should be an array");
		}
	...
	if (mySetting.exists("enabled_for_run_range"))
	{
		enabled_for_this_grid = false;
		Setting& EnabledGrids = mySetting["enabled_for_run_range"];
		if (EnabledGrids.getType()!=Setting::TypeArray)
		{
			throw AngoraInvalidSettingTypeException(EnabledGrids,"should be an array");
		}
		...
		if (EnabledGrids.getLength()!=2)
		{
			throw AngoraInvalidSettingValueException(EnabledGrids,"should have 2 elements");
		}
		if ((int)EnabledGrids[0]>(int)EnabledGrids[1])
		{
			ostringstream firstvalue;
			firstvalue << (int)EnabledGrids[0];
			throw AngoraInvalidSettingValueException(EnabledGrids[1],"should be larger than or equal to "+firstvalue.str());
		}
		if ((GridIndex>=(int)EnabledGrids[0])&&(GridIndex<=(int)EnabledGrids[1]))
		{
			enabled_for_this_grid = true;
		}
	}
