...
int Ctfsf::AddFocusedLaserBeam(const FLBDataType& MyData)
{//adds a focused laser beam to the TF/SF object
	boost::shared_ptr<Cflb> new_flb_ptr(new Cflb(MyData,NumberOfFocusedLaserBeams()));
	FocusedLaserBeams.push_back(new_flb_ptr);
	return NumberOfFocusedLaserBeams()-1;
}
...
void Ctfsf::CorrectE(const int& n)
{
	...
	for (int i=0; i<NumberOfFocusedLaserBeams(); i++)
	{
		FocusedLaserBeams[i]->CorrectE(n);		//apply correction to the main-grid E-field (using Hinc at n+1/2)
	}
...
void Ctfsf::CorrectH(const int& n)
{
	...
	for (int i=0; i<NumberOfFocusedLaserBeams(); i++)
	{
		FocusedLaserBeams[i]->CorrectH(n);		//apply correction to the main-grid H-field (using Einc at n+1)
	}
//	for (int i=0; i<NumberOfFocusedBeams(); i++)
//	{
...
{//write the scattering angles (THETA and PHI) of the scattered PWs into PW_THETA and PW_PHI
	...
	for (int i=0; i<NumberOfFocusedLaserBeams(); i++)
	{
		FocusedLaserBeams[i]->WriteScatteredPWDirections(PW_THETA,PW_PHI);
	}
//	for (int i=0; i<NumberOfFocusedBeams(); i++)
//	{
...
{//write the delays (from the origin) of the scattered PWs into origindelay_array
	...
	for (int i=0; i<NumberOfFocusedLaserBeams(); i++)
	{
		FocusedLaserBeams[i]->WriteScatteredPWDelaysFromOrigin(origindelay_array,FFOriginX,FFOriginY,FFOriginZ);
	}
//	for (int i=0; i<NumberOfFocusedBeams(); i++)
//	{
...
{//write the field amplitudes of the scattered PWs into field_array
	...
	for (int i=0; i<NumberOfFocusedLaserBeams(); i++)
	{
		FocusedLaserBeams[i]->WriteScatteredPWFieldAmplitudes(E_x_array,E_y_array);
	}
//	for (int i=0; i<NumberOfFocusedBeams(); i++)
//	{
