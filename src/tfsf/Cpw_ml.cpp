...
extern Array<float,1> eps_x,eps_z;
extern Array<float,1> cond_e_x,cond_e_z;
...
{
	//put a lower limit on the z-component of the incidence vector
	//the computational burden increases as k approaches grazing incidence (theta=90deg)
	if (abs(k_inc_z)<ANGORA_CPW_ML_MIN_GRAZING_COS_Z)
	{
		throw MLPWGrazingIncidenceException();
	}
//	/** REMOVE LATER **/
...
//TinyVector<double,3> k_E = CosPsi*cross(k_inc_lateral,unit_z)+SinPsi*cross(cross(k_inc_lateral,unit_z),k_inc);
//cout << k_inc(0) << "," << k_inc(1) << "," << k_inc(2) << "," << k_E(0) << "," << k_E(1) << "," << k_E(2) << "," << Data.E0 << endl;
...
/********* This portion of the code may need further testing ******************/
	...
	int num_extra_cells_bottom,num_extra_cells_top;
	if (k_inc_z<0)
	{//incident from the upper half-space
		num_extra_cells_bottom = 1;
		num_extra_cells_top = 4;
	}
	else
	{//incident from the lower half-space
		num_extra_cells_bottom = 4;
		num_extra_cells_top = 1;
	}
	//lowermost cell in the auxiliary grid
	AuxGridLower = max(1,min(PWLowerZ-1,LayerLowerZIndices(1)-1)-num_extra_cells_bottom);
	...
	AuxGridUpper = min(NCELLS_Z + 2*NPML,max(PWUpperZ,LayerLowerZIndices(number_of_layers-1))+num_extra_cells_top);
	...
	AbsorbIndexUpper = AuxGridUpper+1;	//E-field absorption point (upper)
	if (k_inc_z<0)
	{//incident from the upper half-space
		//E-field feed point is 1 cell above the top surface of the TF/SF box or the uppermost layer interface (whichever is higher)
		FeedIndexE = AbsorbIndexUpper-(num_extra_cells_top-1);
		//(H-field feed point is in the same cell- right above it)
		FeedIndexH = FeedIndexE;
	}
	else
	{//incident from the lower half-space
		//E-field feed point is 1 cell above the top surface of the TF/SF box or the uppermost layer interface (whichever is higher)
		FeedIndexE = AbsorbIndexLower+(num_extra_cells_bottom-1);
		//(H-field feed point is in the cell below)
		FeedIndexH = FeedIndexE-1;
	}
/********* This portion of the code may need further testing ******************/
	...
	{
		PWOriginationPoint = (PWFrontX+1),(PWRightY+1),(FeedIndexE-1);
	}
	if ((k_inc_lateral(firstDim)>0)&&(k_inc_lateral(secondDim)<=0))
	{
		PWOriginationPoint = (PWBackX-2),(PWRightY+1),(FeedIndexE-1);
	}
	if ((k_inc_lateral(firstDim)>0)&&(k_inc_lateral(secondDim)>0))
	{
		PWOriginationPoint = (PWBackX-2),(PWLeftY-2),(FeedIndexE-1);
	}
	if ((k_inc_lateral(firstDim)<=0)&&(k_inc_lateral(secondDim)>0))
	{
		PWOriginationPoint = (PWFrontX+1),(PWLeftY-2),(FeedIndexE-1);
	}
	...
	{//incident from the lower half-space
		...
		epsilon_r_t = epsilon_r_upper;
	}

	//calculate grid velocity in the incidence half space
	vp_i = c_i*GridVelocity(THETA_INC,PHI_INC);
	vp_t = c_t*GridVelocity(THETA_INC,PHI_INC);

	//time needed for the plane wave to reach the origin from the plane-wave origination point
	OriginDelay = dot(-k_inc,PWOriginationPoint-PWOrigin)*dx/vp_i;
	...
	{
		epsilon_E(k) = eps_x(eps_x_indices_on_z_axis(k)); /** isotropy assumed! **/	//eps_y could also be used
		cond_E(k) = cond_e_x(cond_e_x_indices_on_z_axis(k)); /** isotropy assumed! **/	//cond_y could also be used
	}
	...
	{
		epsilon_H(k) = eps_z(eps_z_indices_on_z_axis(k)); /** isotropy assumed! **/
		cond_H(k) = cond_e_z(cond_e_z_indices_on_z_axis(k)); /** isotropy assumed! **/
	}
	...
	for (int k=AuxGridLower;k<=AuxGridUpper+1;k++)
	{
		if ((epsilon_E(k)-epsilon_r_i*pow(SinT,2))<=0)
		{
	...
	for (int k=AuxGridLower;k<=AuxGridUpper;k++)
	{
		if ((epsilon_H(k)-epsilon_r_i*pow(SinT,2))<=0)
		{
	...
	waveformHilbert = Data.waveform->HilbertTransform();
	}
	catch (NoHilbertTransform a)
	{
		if (!EvanescentWavePresent)
		{//if there are no evanescent waves, there is no need for the analytic waveform,
		 //so just assign 0 to the imaginary part
			waveformHilbert = Data.waveform->clone();
			waveformHilbert->multiply_amplitude_by(0);
		}
		else
		{//if there are evanescent waves, this waveform cannot be used
			throw NonNarrowbandWaveform();
		}
	}
	...
	for (int k=AuxGridLower;k<=AuxGridUpper+1;k++)
	{
		if (eps_x_indices_on_z_axis(k)!=PEC) /** isotropy assumed! **/
		{
			...
			Ca_h(k) = (1.0-cond_E(k)*dt_g/(2*epsilon_0*(epsilon_E(k)-epsilon_r_i*pow(SinT,2))))
				/(1.0+cond_E(k)*dt_g/(2*epsilon_0*(epsilon_E(k)-epsilon_r_i*pow(SinT,2))));
			Cb_h(k) = (dt_g/dx/epsilon_0)/(epsilon_E(k)-epsilon_r_i*pow(SinT,2))
				/(1.0+cond_E(k)*dt_g/(2*epsilon_0*(epsilon_E(k)-epsilon_r_i*pow(SinT,2))));
			Hz_h(k) = (1/eta_0)*sqrt(epsilon_r_i)*abs(SinT);	//the coefficient Hz/Eh
		}
		else
		{
			Ca_e(k) = -1.0;
	...
	{
		Da_e(k) = 1.0;
		Db_e(k) = (dt_g/dx/mu_0*epsilon_H(k))/(epsilon_H(k)-epsilon_r_i*pow(SinT,2));
		Da_e_pr(k) = (1.0-cond_H(k)*dt_g/(2*epsilon_0*(epsilon_H(k)-epsilon_r_i*pow(SinT,2))))
				/(1.0+cond_H(k)*dt_g/(2*epsilon_0*(epsilon_H(k)-epsilon_r_i*pow(SinT,2))));
		Db_e_pr(k) = (dt_g/dx/mu_0)/(epsilon_H(k)-epsilon_r_i*pow(SinT,2))
				/(1.0+cond_H(k)*dt_g/(2*epsilon_0*(epsilon_H(k)-epsilon_r_i*pow(SinT,2))));
		Da_h(k) = 1.0;
		Db_h(k) = dt_g/dx/mu_0;
		A_pr(k) = epsilon_H(k)+cond_H(k)*dt_g/(2*epsilon_0);
		B_pr(k) = epsilon_H(k)-cond_H(k)*dt_g/(2*epsilon_0);
		Ez_e(k) = -eta_0*sqrt(epsilon_r_i)*abs(SinT)/epsilon_H(k);	//the coefficient Ez/He
		Ez_e_pr(k) = -eta_0*sqrt(epsilon_r_i)*abs(SinT);	//the coefficient Ez/He'
	}
	...
	Sc_1D_upper = c/sqrt(epsilon_E(AbsorbIndexUpper)-epsilon_r_i*pow(SinT,2))*dt_g/dx;
	Sc_1D_lower = c/sqrt(epsilon_E(AbsorbIndexLower)-epsilon_r_i*pow(SinT,2))*dt_g/dx;
...
//calculates the reduced time step to be used in the auxiliary time grid
{
	...
	for (int k=AuxGridLower+1; k<=AuxGridUpper;k++)
	{
		c1 = sqrt(1/epsilon_0/epsilon_E(k)
			*epsilon_H(k)/mu_0/abs(epsilon_H(k)-epsilon_r_i*pow(SinT,2)));
		c2 = sqrt(1/epsilon_0/epsilon_E(k)
			*epsilon_H(k-1)/mu_0/abs(epsilon_H(k-1)-epsilon_r_i*pow(SinT,2)));
		c3 = sqrt(1/epsilon_0/abs(epsilon_E(k)-epsilon_r_i*pow(SinT,2))
			*1/mu_0);
...
//		waveformH_analytic(n)=Data.E0*1/(eta_0/sqrt(epsilon_r_i))*
//			(Data.waveform->Value((n-0.5)*dt_g+OriginDelay+distance/vp_i+time_offset)
//			+ii*(waveformHilbert->Value((n-0.5)*dt_g+OriginDelay+distance/vp_i+time_offset))); //imag. part is Hilb. transform
//	}
//
...
{
	return -SinPsi*Data.E0*1/(eta_0/sqrt(epsilon_r_i))*
	(Data.waveform->Value((m-0.5)*dt_g+OriginDelay+ANGORA_CPW_ML_SPATIAL_OFFSET/vp_i+ANGORA_CPW_ML_TIME_OFFSET+get_initial_time_value())
 +ii*(waveformHilbert->Value((m-0.5)*dt_g+OriginDelay+ANGORA_CPW_ML_SPATIAL_OFFSET/vp_i+ANGORA_CPW_ML_TIME_OFFSET+get_initial_time_value())));
}

complex<double> Cpw_ml::Incident_Hh(const double& m)
{
	return CosPsi*CosT*Data.E0*1/(eta_0/sqrt(epsilon_r_i))*
	(Data.waveform->Value((m-0.5)*dt_g+OriginDelay+ANGORA_CPW_ML_SPATIAL_OFFSET/vp_i+ANGORA_CPW_ML_TIME_OFFSET+get_initial_time_value())
 +ii*(waveformHilbert->Value((m-0.5)*dt_g+OriginDelay+ANGORA_CPW_ML_SPATIAL_OFFSET/vp_i+ANGORA_CPW_ML_TIME_OFFSET+get_initial_time_value())));
}
...
void Cpw_ml::DetermineEvanescentRegions()
{
	...
	while (k<=AuxGridUpper)
	{
		if (IsEvanescent_z(k))	//start block
		{
			...
			for(;(k<AuxGridUpper)&&IsEvanescent_z(k+1);k++);	//increment k until the end of the evanescent layer
...
{//write the scattering angle (THETA and PHI) of the scattered PW into PW_THETA and PW_PHI
	...
	//then, the transmission (if the plane wave is not evanescent in the transmission half space)
	//theta
	PW_THETA.resizeAndPreserve(PW_THETA.size()+1);
	if (abs(sin(THETA_INC)/sqrt(epsilon_r_t/epsilon_r_i))<1)
	{
		if (k_inc_z<0)
		{//incident from the upper half-space
			// calculate the transmission theta angle using Snell's law
			PW_THETA(PW_THETA.size()-1) = M_PI+asin(sin(THETA_INC)/sqrt(epsilon_r_t/epsilon_r_i)); //always between pi/2 and 3*pi/2
		}
		else
		{//incident from the lower half-space
			// calculate the transmission theta angle using Snell's law
			//"incidence" angle is pi-theta (w.r.t. the -z axis)
			PW_THETA(PW_THETA.size()-1) = -asin(sin(M_PI-THETA_INC)/sqrt(epsilon_r_t/epsilon_r_i)); //always between -pi/2 and pi/2
		}
	}
	//phi
	PW_PHI.resizeAndPreserve(PW_PHI.size()+1);
...
{//write the delay (from the origin) of the scattered PW into origindelay_array
	...
	if (k_inc_z<0)
	{//incident from the upper half-space
		PWScatteredFieldPoint = (PWOriginationPoint(firstDim)),(PWOriginationPoint(secondDim)),(FeedIndexE+1);	//go 2 cells into the scattered-field (SF) region
	}
	else
	{//incident from the lower half-space
		PWScatteredFieldPoint = (PWOriginationPoint(firstDim)),(PWOriginationPoint(secondDim)),(FeedIndexE-3);	//go 2 cells into the scattered-field (SF) region
	}
	TinyVector<double,3> k_spec(k_inc);	//specular reflection
	k_spec(thirdDim) *= -1;	//reverse the z-component
	double FFdelay = dot(k_spec,PWScatteredFieldPoint-FFOrigin)*dx/vp_i;	//calculate the delay w.r.t the TRANSFORMER origin
	origindelay_array.resizeAndPreserve(origindelay_array.size()+1);
	origindelay_array(origindelay_array.size()-1) = FFdelay;

	//then, for the transmission (if the plane wave is not evanescent in the transmission half space)
	if (abs(sin(THETA_INC)/sqrt(epsilon_r_t/epsilon_r_i))<1)
	{
		...
		if (k_inc_z<0)
		{//incident from the upper half-space
			PWScatteredFieldPoint = (PWOriginationPoint(firstDim)),(PWOriginationPoint(secondDim)),(AbsorbIndexLower-1);	//take the field value at the absorption point
			theta_trans = M_PI+asin(sin(THETA_INC)/sqrt(epsilon_r_t/epsilon_r_i)); //always between pi/2 and 3*pi/2
		}
		else
		{//incident from the lower half-space
			PWScatteredFieldPoint = (PWOriginationPoint(firstDim)),(PWOriginationPoint(secondDim)),(AbsorbIndexUpper-1);	//take the field value at the absorption point
			theta_trans = -asin(sin(M_PI-THETA_INC)/sqrt(epsilon_r_t/epsilon_r_i)); //always between -pi/2 and pi/2
		}
		k_trans(firstDim) = sin(theta_trans)*cos(PHI_INC); //note that the transmitted phi angle is the same as PHI_INC
		k_trans(secondDim) = sin(theta_trans)*sin(PHI_INC); //note that the transmitted phi angle is the same as PHI_INC
		k_trans(thirdDim) = cos(theta_trans);
		FFdelay = dot(k_trans,PWScatteredFieldPoint-FFOrigin)*dx/vp_t;	//calculate the delay w.r.t the TRANSFORMER origin
		origindelay_array.resizeAndPreserve(origindelay_array.size()+1);
		origindelay_array(origindelay_array.size()-1) = FFdelay;
	}
}
...
{//write the field amplitude of the scattered PWs into E_x_array and E_x_array
	//first, the specular reflection
	...
	if (k_inc_z<0)
	{//incident from the upper half-space
		E_x_array(E_x_array.size()-1) = k_Ee(firstDim)*real(AuxGrid_Ee(FeedIndexE+2))
									+ k_Eh(firstDim)*real(AuxGrid_Eh(FeedIndexE+2));
		//y-component of the E-field
		E_y_array(E_y_array.size()-1) = k_Ee(secondDim)*real(AuxGrid_Ee(FeedIndexE+2))
									+ k_Eh(secondDim)*real(AuxGrid_Eh(FeedIndexE+2));
	}
	else
	{//incident from the lower half-space
		E_x_array(E_x_array.size()-1) = k_Ee(firstDim)*real(AuxGrid_Ee(FeedIndexE-2))
	...
	if (abs(sin(THETA_INC)/sqrt(epsilon_r_t/epsilon_r_i))<1)
	{
		...
		{//incident from the upper half-space
			//write the TANGENTIAL component (either x or y) of the E-field
			//x-component of the E-field
			E_x_array(E_x_array.size()-1) = k_Ee(firstDim)*real(AuxGrid_Ee(AbsorbIndexLower))
										+ k_Eh(firstDim)*real(AuxGrid_Eh(AbsorbIndexLower));
			//y-component of the E-field
			E_y_array(E_y_array.size()-1) = k_Ee(secondDim)*real(AuxGrid_Ee(AbsorbIndexLower))
										+ k_Eh(secondDim)*real(AuxGrid_Eh(AbsorbIndexLower));
		}
		else
		{//incident from the lower half-space
			//write the TANGENTIAL component (either x or y) of the E-field
			//x-component of the E-field
			E_x_array(E_x_array.size()-1) = k_Ee(firstDim)*real(AuxGrid_Ee(AbsorbIndexUpper))
										+ k_Eh(firstDim)*real(AuxGrid_Eh(AbsorbIndexUpper));
			//y-component of the E-field
			E_y_array(E_y_array.size()-1) = k_Ee(secondDim)*real(AuxGrid_Ee(AbsorbIndexUpper))
										+ k_Eh(secondDim)*real(AuxGrid_Eh(AbsorbIndexUpper));
		}
	}
}
