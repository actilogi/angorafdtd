...
//		InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
		...
		throw AngoraInvalidArgumentExceptionWithType<double>(func_name,Data.ap_half_angle,
			"(ap_half_angle should be between 0 and 90deg)");
	}
...
//						InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
	...
	else if (Data.angular_discretization=="radial")
	{
		...
		//Gauss-Legendre parameters for rho quadrature [ rho=sin(theta), -sin(ap_half_angle)<rho<sin(ap_half_angle) ]
		GLx.resize(N_X1); //Gauss-Legendre coordinates
		GLw.resize(N_X1); //Gauss-Legendre weights

		//calculate GL quadrature rule for the sin(theta) values (between [-sin(ap_half_angle),sin(ap_half_angle)])
		gaussquadrule(N_X1,GLx,GLw);
		//rescale the positions and weights to the correct ranges of sin(theta) (between [-sin(ap_half_angle),sin(ap_half_angle)])
