...
{//updates the E-field components at time index n
	//Update Ee,Eh
	for (k=AbsorbIndexLower+1;k<=AbsorbIndexUpper-1;k++)
	{
	...
	//Update Ez
	for (k=AbsorbIndexLower;k<=AbsorbIndexUpper-1;k++)
	{
	...
	//Apply (TF/SF) correction for Einc: (NOTE: Incidence half space is assumed LOSSLESS)
	//correct Ee
	AuxGrid_Ee(FeedIndexE) += -Cb_e(FeedIndexE)*Incident_He(n*timefactor+timeindex);
	//correct Eh
	AuxGrid_Eh(FeedIndexE) += -Cb_h(FeedIndexE)*Incident_Hh(n*timefactor+timeindex);
	...
	//Upper absorbing point (assuming the wave is PROPAGATING at this point !!!! May generalize to evanescent waves later)
	if (!IsEvanescent_xy(AbsorbIndexUpper))	//if wave is not evanescent at either below or above this position, perform absorption
	{
		//absorb Ee
		AuxGrid_Ee(AbsorbIndexUpper) = (-1/(1/Sc_1D_upper+2+Sc_1D_upper))*(
			(1/Sc_1D_upper-2+Sc_1D_upper)*(AuxGrid_Ee(AbsorbIndexUpper-2)+PreviousEeUpper(0,-1))
			+ 2*(Sc_1D_upper-1/Sc_1D_upper)*(PreviousEeUpper(0,0)+PreviousEeUpper(2,0)-AuxGrid_Ee(AbsorbIndexUpper-1)-PreviousEeUpper(1,-1))
			- 4*(1/Sc_1D_upper+Sc_1D_upper)*PreviousEeUpper(1,0)) - PreviousEeUpper(2,-1);
		//absorb Eh
		AuxGrid_Eh(AbsorbIndexUpper) = (-1/(1/Sc_1D_upper+2+Sc_1D_upper))*(
			(1/Sc_1D_upper-2+Sc_1D_upper)*(AuxGrid_Eh(AbsorbIndexUpper-2)+PreviousEhUpper(0,-1))
			+ 2*(Sc_1D_upper-1/Sc_1D_upper)*(PreviousEhUpper(0,0)+PreviousEhUpper(2,0)-AuxGrid_Eh(AbsorbIndexUpper-1)-PreviousEhUpper(1,-1))
		...
		PreviousEeUpper(Range(0,2),0) = AuxGrid_Ee(Range(AbsorbIndexUpper,AbsorbIndexUpper-2,-1));	//record the current values as history
		PreviousEhUpper(Range(0,2),-1) = PreviousEhUpper(Range(0,2),0);	//shift toward the future by one step
		PreviousEhUpper(Range(0,2),0) = AuxGrid_Eh(Range(AbsorbIndexUpper,AbsorbIndexUpper-2,-1));	//record the current values as history
	}

	//Lower absorbing point	(assuming the wave is PROPAGATING at this point !!!! May generalize to evanescent waves later)
	if (!IsEvanescent_xy(AbsorbIndexLower))	//if wave is not evanescent at either below or above this position, perform absorption
	{
		//absorb Ee
		AuxGrid_Ee(AbsorbIndexLower) = (-1/(1/Sc_1D_lower+2+Sc_1D_lower))*(
			(1/Sc_1D_lower-2+Sc_1D_lower)*(AuxGrid_Ee(AbsorbIndexLower+2)+PreviousEeLower(0,-1))
			+ 2*(Sc_1D_lower-1/Sc_1D_lower)*(PreviousEeLower(0,0)+PreviousEeLower(2,0)-AuxGrid_Ee(AbsorbIndexLower+1)-PreviousEeLower(1,-1))
			- 4*(1/Sc_1D_lower+Sc_1D_lower)*PreviousEeLower(1,0)) - PreviousEeLower(2,-1);
		//absorb Eh
		AuxGrid_Eh(AbsorbIndexLower) = (-1/(1/Sc_1D_lower+2+Sc_1D_lower))*(
			(1/Sc_1D_lower-2+Sc_1D_lower)*(AuxGrid_Eh(AbsorbIndexLower+2)+PreviousEhLower(0,-1))
			+ 2*(Sc_1D_lower-1/Sc_1D_lower)*(PreviousEhLower(0,0)+PreviousEhLower(2,0)-AuxGrid_Eh(AbsorbIndexLower+1)-PreviousEhLower(1,-1))
		...
		PreviousEeLower(Range(0,2),0) = AuxGrid_Ee(Range(AbsorbIndexLower,AbsorbIndexLower+2));	//record the current values as history
		PreviousEhLower(Range(0,2),-1) = PreviousEhLower(Range(0,2),0);	//shift toward the future by one step
		PreviousEhLower(Range(0,2),0) = AuxGrid_Eh(Range(AbsorbIndexLower,AbsorbIndexLower+2));	//record the current values as history
	}
}
...
{//updates the H-field components at time index n
	...
	//Apply (TF/SF) correction for Hinc: (NOTE: Incidence half space is assumed LOSSLESS)
	//correct He' first
	AuxGrid_He_pr(FeedIndexH) += -Db_e_pr(FeedIndexH)*Incident_Ee(n*timefactor+timeindex);
	//then obtain He from He' (linear dependence, since incidence half space is lossless => er=er')
	AuxGrid_He(FeedIndexH) = epsilon_H(FeedIndexH)*AuxGrid_He_pr(FeedIndexH);
	//finally correct Hh
	AuxGrid_Hh(FeedIndexH) += -Db_h(FeedIndexH)*Incident_Eh(n*timefactor+timeindex);
}
...
void Cpw_ml::UpdateEvanescentE()
{
	for (i=0; i<NumOfEvanescentLayers; i++)
	{
		if (EvanescentLayers(i).homogeneous_at_interfaces)
		{
			...
			if (info!=0)
			{
				throw AngoraDeveloperException("Error in Cpw_ml (multilayer plane-wave class): Matrix inversion failed in evanescent layer.");
			}
