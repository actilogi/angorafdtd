...
/** TODO: This value is pretty random, make this more sensible **/
#define ANGORA_CPW_ML_MIN_GRAZING_COS_Z 1e-4


class MLPWGrazingIncidenceException: public AngoraException
{// the exception class for too low grazing angles
	public:
	virtual const string getError() const
	{
		return "error: Plane-wave grazing angle (with respect to layer interfaces) too close to zero.";
	}
};

class NonNarrowbandWaveform: public AngoraException
{// the exception class for non-narrowband waveforms in the presence of evanescent plane waves
	public:
	virtual const string getError() const
	{
		return "error: Evanescent plane waves cannot be created with non-narrowband waveforms.";
	}
};
...
class Cpw_ml : public Cpw
{
 ...
 private:
	 ...
	 double waveform(double t, double tau);
 	 ...
 	 //grid velocity in the incidence and transmission half spaces
	 double vp_i,vp_t;
 	 //continuous-space propagation velocity in the incidence and transmission half spaces
	 double c_i,c_t;
	 //relative permittivity in the incidence and transmission half spaces
	 double epsilon_r_i,epsilon_r_t;
...
//	 Array<complex<double>,1> waveformE_analytic,waveformH_analytic;	//analytic waveforms of the incident homogeneous plane wave in the uppermost layer
	 ...
	 int FeedIndexE,FeedIndexH,AbsorbIndexLower,AbsorbIndexUpper;	//E and H-field array indices or the feed and absorbing points for the plane wave in 1-D grid
