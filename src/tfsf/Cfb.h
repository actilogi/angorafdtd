...
class FBGrazingIncidenceException: public AngoraException
{// the exception class for beams cutting across planar layer interfaces
	public:
	virtual const string getError() const
	{
		return "Error in Cfb (focused beam class): Focused-beam grazing angle too low. Try larger \"theta\" or narrower aperture.";
	}
};
...
// 		ap_half_angle(23.5782*M_PI/180),
...
// 		f(0.01),
	...
	int n_1; //for "cartesian": number of x-direction cosines;  For "radial": number of theta between -ap_half_angle (not included) and ap_half_angle (not included)
...
class Cfb
{
 ...
 private:
	 ...
	 //relative permittivity and permeability in the incidence half spaces
	 ...
	 double theta_pw,phi_pw,psi_pw;	//incidence angles and polarization angle for a single plane-wave component
