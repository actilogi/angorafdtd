...
#include "check_dispersion.h"

//function that returns all the valid settings
#include "valid_settings.h"
...
//		read_obj(fdtdconfig,validsettings,Shapes,Materials);
		...
		if (!check_mode)
		{
			...
			double CPML_feature_size; //in grid cells
			try{
				if (read_length_from_group<double>(fdtdconfig.getRoot(),"cpml_feature_size",CPML_feature_size))
				{//returned true if given in meters:
					CPML_feature_size /= dx;
				}
			}
...
//			cout << endl << "Accuracy in grid " << GridIndex << " is " << accuracy << " dB." << endl;
			...
			cout << endl << "In run " << GridIndex << ":" << endl
				...
				<< " Number of focused laser beams : " << TFSF.NumberOfFocusedLaserBeams() << endl
		...
		if (!check_mode)//don't run simulation or do post processing in check_mode
		{
			...
			if ((rank==0)&&logging_enabled)
			{
				read_logging_data(Estimator,fdtdconfig,simulation_starting_time);
			}
	...
	catch( MaterialPropertyDoesNotExist& error )
	{
		throw AngoraDeveloperException(error.getError());
	}
