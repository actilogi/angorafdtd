...
void read_shapes(Cshapes &Shapes, const Config& fdtdconfig, const Config& validsettings)
{
	...
	if (fdtdconfig.exists(shape_setting_path))
	{
		...
		if (SettingEnabledForGrid(shape_settings))		//apply only if enabled for this grid
		{
			...
			if (shape_settings.exists(rect_boxes_setting_path))
			{
				...
				for (int rectangular_box_index=0; rectangular_box_index<num_of_rectangular_boxes; rectangular_box_index++)
				{
					...
					if (SettingEnabledForGrid(rectangular_box_settings))		//apply only if enabled for this grid
					{
						...
						//back_x
						double back_x;
						if (!read_length_from_group<double>(rectangular_box_settings,"back_coord_x",back_x))
						{//returned false: given in grid cells
							back_x *= dx;
						}
						//front_x
						double front_x;
						if (!read_length_from_group<double>(rectangular_box_settings,"front_coord_x",front_x))
						{//returned false: given in grid cells
							front_x *= dx;
						}
						//left_y
						double left_y;
						if (!read_length_from_group<double>(rectangular_box_settings,"left_coord_y",left_y))
						{//returned false: given in grid cells
							left_y *= dx;
						}
						//right_y
						double right_y;
						if (!read_length_from_group<double>(rectangular_box_settings,"right_coord_y",right_y))
						{//returned false: given in grid cells
							right_y *= dx;
						}
						//lower_z
						double lower_z;
						if (!read_length_from_group<double>(rectangular_box_settings,"lower_coord_z",lower_z))
						{//returned false: given in grid cells
							lower_z *= dx;
						}
						//upper_z
						double upper_z;
						if (!read_length_from_group<double>(rectangular_box_settings,"upper_coord_z",upper_z))
						{//returned false: given in grid cells
							upper_z *= dx;
						}
			...
			if (shape_settings.exists(spheres_setting_path))
			{
				...
				for (int sphere_index=0; sphere_index<num_of_spheres; sphere_index++)
				{
					...
					if (SettingEnabledForGrid(sphere_settings))		//apply only if enabled for this grid
					{
						...
						//center_x
						double center_x;
						if (!read_length_from_group<double>(sphere_settings,"center_coord_x",center_x))
						{//returned false: given in grid cells
							center_x *= dx;
						}
						//center_y
						double center_y;
						if (!read_length_from_group<double>(sphere_settings,"center_coord_y",center_y))
						{//returned false: given in grid cells
							center_y *= dx;
						}
						//center_z
						double center_z;
						if (!read_length_from_group<double>(sphere_settings,"center_coord_z",center_z))
						{//returned false: given in grid cells
							center_z *= dx;
						}
						//get the radius of the sphere
						double radius;
						if (!read_length_from_group<double>(sphere_settings,"radius",radius))
						{//returned false: given in grid cells
							radius *= dx;
						}
