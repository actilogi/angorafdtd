...
const_Cshape_shared_ptr Cshapes::operator[] (const string& shape_tag) const
{
	map<string,Cshape_shared_ptr>::const_iterator map_it = NamedShapes.find(shape_tag);
	if (map_it==NamedShapes.end())
...
{//creates a planar layer with the given string tag
	...
	{
		//create shape
		Cshape_shared_ptr new_shape_ptr(new Cplanarlayer(my_min_coord,my_max_coord,my_orientation));
//		ShapePtrs.push_back(new_shape_ptr);
		//add the shape with the given tag
		NamedShapes.insert(pair<string,Cshape_shared_ptr>(shape_tag,new_shape_ptr));
	}
	else
...
{//creates a rectangular box with the given string tag
	...
	{
		//create shape
		Cshape_shared_ptr new_shape_ptr(new Crectbox(my_back_x,my_front_x,my_left_y,my_right_y,my_lower_z,my_upper_z));
		//add the shape with the given tag
		NamedShapes.insert(pair<string,Cshape_shared_ptr>(shape_tag,new_shape_ptr));
	}
	else
...
{//creates a sphere with the given string tag
	...
	{
		//create shape
		Cshape_shared_ptr new_shape_ptr(new Csphere(my_center_x,my_center_y,my_center_z,my_radius));
		//add the shape with the given tag
		NamedShapes.insert(pair<string,Cshape_shared_ptr>(shape_tag,new_shape_ptr));
	}
	else
