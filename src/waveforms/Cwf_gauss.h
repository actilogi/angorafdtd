...
{//base class for non-modulated Gaussian waveforms
 public:
	...
	double Integral(const double& t) const;
	complex<double> FourierTransform(const double& w) const;
	double HilbertTransform(const double& t) const;
	double PhaseShift(const double& extra_phase,const double& t) const;

	double w_min_20() const {return 0;};
	double w_max_20() const {return 2.146/tau;};
	double w_min_40() const {return 0;};
	double w_max_40() const {return 3.035/tau;};
...
//			InvalidNumericArgumentException<double> exc(__PRETTY_FUNCTION__,1);
	...
	double w_max_20() const {return 2.764/tau;};
...
{//base class for modulated Gaussian waveforms
 public:
	...
	double w_max_20() const {return 2*M_PI*f_0 + 2.146/tau;};
