...
void Cimg::ConstructImage()
{
	...
	{// only node 0 constructs the image
		...
		int lambda_size = dataset.getSpace().getSimpleExtentNpoints();
		...
		int dir1_size = dataset.getSpace().getSimpleExtentNpoints();
		...
		int dir2_size = dataset.getSpace().getSimpleExtentNpoints();
...
//// cout << PW_y_phasorarray << endl;
        ...
        for (int PW=0; PW<numofPWs; PW++)
        {
            ...
            if (pw_theta_within_NA)
            {
                ...
                for (int k=0; k<lambda_size; k++)
                {
                    ...
                    radiometric_factor = sqrt((nimg*dir3_pw)/(nobj*dir3M_pw))/Mpr;
		...
		if (Data.output_data_item_exists("E_x_unsca"))
		{
			dataset=image_file.createDataSet("E_x_unsca_r", PredType::NATIVE_DOUBLE, dspace, plist);
			//write the real part
			temp = real(E_pw_x);
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
			//write the imaginary part
			dataset=image_file.createDataSet("E_x_unsca_i", PredType::NATIVE_DOUBLE, dspace, plist);
			temp = imag(E_pw_x);
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
		}
		/** y-component of the incident E-field **/
		if (Data.output_data_item_exists("E_y_unsca"))
		{
			dataset=image_file.createDataSet("E_y_unsca_r", PredType::NATIVE_DOUBLE, dspace, plist);
			//write the real part
			temp = real(E_pw_y);
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
			//write the imaginary part
			dataset=image_file.createDataSet("E_y_unsca_i", PredType::NATIVE_DOUBLE, dspace, plist);
			temp = imag(E_pw_y);
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
		}
		/** z-component of the incident E-field **/
		if (Data.output_data_item_exists("E_z_unsca"))
		{
			dataset=image_file.createDataSet("E_z_unsca_r", PredType::NATIVE_DOUBLE, dspace, plist);
			//write the real part
			temp = real(E_pw_z);
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
			//write the imaginary part
			dataset=image_file.createDataSet("E_z_unsca_i", PredType::NATIVE_DOUBLE, dspace, plist);
			temp = imag(E_pw_z);
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
		}
		...
		if (Data.output_data_item_exists("intensity_tot"))
		{
			...
			temp = (nimg/eta_0)*pow2(abs(Ex+E_pw_x))+pow2(abs(Ey+E_pw_y))+pow2(abs(Ez+E_pw_z)); //in W/m^2
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
		}
...
//			dataset.write(temp2D.data(),PredType::NATIVE_DOUBLE);
		...
		if (Data.output_data_item_exists("intensity_sca"))
		{
			...
			temp = (nimg/eta_0)*pow2(abs(Ex))+pow2(abs(Ey))+pow2(abs(Ez)); //in W/m^2
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
		}

		/** incident intensity **/
		if (Data.output_data_item_exists("intensity_unsca"))
		{
			dataset=image_file.createDataSet("intensity_unsca", PredType::NATIVE_DOUBLE, dspace, plist);
			temp = (nimg/eta_0)*pow2(abs(E_pw_x))+pow2(abs(E_pw_y))+pow2(abs(E_pw_z)); //in W/m^2
			dataset.write(temp.data(),PredType::NATIVE_DOUBLE);
		}
