...
void Celectricdipole::ApplyElectricDipole(const int& n)
{
	if (ElectricDipoleBelongsToNode)
	{
		if (Orientation=="x_directed")
		{
			Ex(SourceCell) += - Cb_X(SourceCell)*dx*moment_to_current_density*waveform_value(n);
		}
		else if (Orientation=="y_directed")
		{
			Ey(SourceCell) += - Cb_Y(SourceCell)*dx*moment_to_current_density*waveform_value(n);
		}
		else if (Orientation=="z_directed")
		{
			Ez(SourceCell) += - Cb_Z(SourceCell)*dx*moment_to_current_density*waveform_value(n);
		}
	}
}
