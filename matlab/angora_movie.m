...
maxvalue_in_file = fread(moviefile ,1,'double');
minvalue_in_file = fread(moviefile ,1,'double');    
...
% override the max. and min. values? (if recording is in double)
if (numofbytes==1)
    maxvalue = maxvalue_in_file;
    minvalue = minvalue_in_file;
end
...
for i=1:num_of_properties
    ...
    % array.
    if ((version_major>0)||((version_major==0)&&(version_minor>=16)))
        property_array(:,:,i)= fread(moviefile ,[length_2,length_1],'float');
    else
...
if (strcmp(units,'mm'))
    unit_label = 'mm';
    unit_factor = 1e3;
else if (strcmp(units,'um'))
    unit_label = '\mum';
    unit_factor = 1e6;
end;end
for n=1:length(frameoffsets)
    H=image(width_range*unit_factor,height_range*unit_factor,zeros(plot_height,plot_width),'CDataMapping','direct');
	fontsize = 16;
    set(gca,'LineWidth',1,'FontName','Times','FontSize',fontsize);
    if (~isempty(strfind(moviefilename,'xz')))
        xlabel(['\it x \rm (' unit_label ')'],'FontSize',fontsize);
        ylabel(['\it z \rm (' unit_label ')'],'FontSize',fontsize);
    else if (~isempty(strfind(moviefilename,'yz')))
        xlabel(['\it y \rm (' unit_label ')'],'FontSize',fontsize);
        ylabel(['\it z \rm (' unit_label ')'],'FontSize',fontsize);
    else if (~isempty(strfind(moviefilename,'xy')))
        xlabel(['\it x \rm (' unit_label ')'],'FontSize',fontsize);
        ylabel(['\it y \rm (' unit_label ')'],'FontSize',fontsize);
    end,end,end            
